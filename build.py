#!/usr/bin/env python3
"""Build the personal website from markdown articles with GEO optimization."""

import os, re, json

ARTICLES_DIR = '/Users/chiryyu/Documents/涉外律师/新文章'
SITE_DIR = '/Users/chiryyu/Documents/涉外律师/website'
ARTICLES_OUT = os.path.join(SITE_DIR, 'articles')
LAST_UPDATED = '2026-08-31'
os.makedirs(ARTICLES_OUT, exist_ok=True)

META = {
    '01_泰国税务争议应对.md': {
        'slug': '01-thailand-tax-dispute', 'region': '泰国', 'tag': '税务合规',
        'date': '2026-06-07', 'desc': '泰国税务局2025年追缴税款超480亿泰铢，转移定价稽查飙升至35%。系统解析从稽查通知到行政诉讼的全流程应对策略。',
        'faq': [
            ('泰国税务局稽查的触发机制是什么？','大数据交叉比对、电子发票系统(e-Tax Invoice)和金融机构信息共享机制，使泰国税务局能精准锁定稽查目标。转移定价、常设机构认定、预提税是当前三大重点稽查领域。'),
            ('收到泰国税务稽查通知后应该怎么做？','收到通知后48小时内应成立内部应对小组、委托税务律师、启动文件隔离程序，避免向稽查官员提交未经审核的文件。'),
            ('泰国税务争议可以通过和解解决吗？','可以。税法税款本金不可减免，但罚金和附加费可通过和解减免。分期缴税是谈判重点。'),
            ('中资企业如何预防泰国税务争议？','建议做好五大支柱：关联交易文档提前准备、预约定价协议(APA)申请、常设机构风险主动管理、受益所有人合规文件、定期税务健康检查。'),
        ],
        'related': ['17-thailand-ma','09-thailand-factory-act','11-thailand-pdpa'],
    },
    '02_柬埔寨QIP申请实务.md': {
        'slug': '02-cambodia-qip', 'region': '柬埔寨', 'tag': '投资法',
        'date': '2026-06-08', 'desc': '2021年新投资法从"正面清单"转向"负面清单+激励分级"。拆解QIP从申请流程、税收优惠规则到持续合规义务的全流程操作要点。',
        'faq': [
            ('柬埔寨QIP申请需要多长时间？','文件齐全的情况下，柬埔寨发展理事会(CDC)审批QIP的周期约2-4个月。若涉及环保审批，可能延长至6个月以上。'),
            ('外资企业申请柬埔寨QIP的最低投资额是多少？','通常不低于50万美元，具体行业有差异。经济特区(SEZ)内的QIP项目门槛相对较低。'),
            ('柬埔寨QIP的税收优惠有哪些？','QIP项目享受3-9年的利润税免税期、进口设备免关税、特别折旧（40%资产原值）、股息预提税豁免等激励。'),
            ('柬埔寨QIP资格可能被撤销吗？','可以。提供虚假材料、连续两年未启动投资、违反环保/劳工法规、未经批准转让免税设备等均可能触发QIP撤销，需补缴全部已享受的税收优惠。'),
        ],
        'related': ['18-cambodia-sez','06-cambodia-construction','12-cambodia-labor-law'],
    },
    '03_泰国刑事拘留权利保护.md': {
        'slug': '03-thailand-criminal-defense', 'region': '泰国', 'tag': '刑事辩护',
        'date': '2026-06-09', 'desc': '从传唤到终审的12个关键节点，拘留阶段的六项核心权利，四大常见涉刑类型辩护要点，以及家属外部支持指南。',
        'faq': [
            ('中国公民在泰国被刑事拘留后有哪些权利？','享有律师在场权、翻译权、领事通知与探视权、保持沉默权、医疗保障权、禁止酷刑等六项核心权利。'),
            ('在泰国被拘留后如何申请保释？','保释由法院决定，需要保释金+2-4名泰籍担保人+交出护照+定期报到。外国人因逃跑风险评估较高，保释难度天然高于泰籍嫌疑人。'),
            ('中国驻泰国使领馆能提供什么帮助？','领事官员可以了解案情、协助聘请律师、联系家属、确保人道权益。但不能代理案件、提供具体法律意见或干预泰国司法程序。'),
        ],
        'related': ['01-thailand-tax-dispute','17-thailand-ma','13-malaysia-amla'],
    },
    '04_马来西亚PDPA执法风暴.md': {
        'slug': '04-malaysia-pdpa', 'region': '马来西亚', 'tag': '数据合规',
        'date': '2026-06-10', 'desc': '2024年PDPA修订后处罚上限提升至100万令吉+3年监禁。与GDPR和泰国PDPA横向对比，解析执法趋势与中资企业实操合规路径。',
        'faq': [
            ('马来西亚PDPA 2024修订的核心变化是什么？','数据保护官(DPO)强制化、数据泄露72小时强制通知、跨境数据传输机制多元化（SCC+BCR+白名单）、数据处理器直接责任、处罚上限提升至100万令吉+3年监禁。'),
            ('中资企业向中国传输马来西亚员工数据是否合规？','需满足条件之一：白名单、标准合同条款(SCC)、集团约束性规则(BCR)、或取得员工明确书面同意。目前中国不在白名单中。'),
        ],
        'related': ['11-thailand-pdpa','14-romania-gdpr','07-vietnam-data-localization'],
    },
    '05_罗马尼亚劳动法实务.md': {
        'slug': '05-romania-labor-law', 'region': '罗马尼亚', 'tag': '劳动法',
        'date': '2026-06-11', 'desc': '罗马尼亚劳动法受欧盟指令深度影响，对雇员保护远超中国标准。详解雇佣、解雇、加班、工会谈判的操作红线和合规要点。',
        'faq': [
            ('在罗马尼亚解雇员工有哪些法律限制？','解雇须基于法定理由并严格遵循程序。纪律解雇须在知悉违规后6个月内完成调查+书面告知+5天答辩期+工会监督。程序瑕疵即可导致解雇无效。'),
            ('罗马尼亚加班费的法律标准是什么？','加班须在30日内支付不低于正常时薪175%的加班费，或经员工书面同意以补休替代（60日内使用）。夜班/周末/公共假日工作通常为200%-300%。'),
        ],
        'related': ['14-romania-gdpr','12-cambodia-labor-law','16-indonesia-omnibus'],
    },
    '06_柬埔寨建筑施工许可.md': {
        'slug': '06-cambodia-construction', 'region': '柬埔寨', 'tag': '工程法',
        'date': '2026-06-12', 'desc': '施工许可不全→工程停工、备案无法通过→无法结算尾款→工程款拖欠→无力整改的恶性循环。拆解从三证体系到纠纷救济的全流程。',
        'faq': [
            ('柬埔寨建筑施工需要哪些许可证？','三证体系：开工许可证(Permit to Commence)、竣工证(Certificate of Completion)、使用证(Certificate of Occupancy)。缺任何一证均属违规。'),
            ('柬埔寨工程款纠纷如何解决？','可通过建设委员会调解、SIAC/HKIAC国际仲裁（柬埔寨已加入纽约公约）、或法院诉讼解决。推荐在合同中约定国际仲裁条款。'),
        ],
        'related': ['02-cambodia-qip','18-cambodia-sez','12-cambodia-labor-law'],
    },
    '07_越南数据本地化合规.md': {
        'slug': '07-vietnam-data-localization', 'region': '越南', 'tag': '数据合规',
        'date': '2026-06-13', 'desc': '第55号法令2026年生效，越南成为东盟数据本地化最严国家。详解触发本地化的"三步审查法"、跨境传输审批机制与中资企业合规路径。',
        'faq': [
            ('越南第55号法令的触发条件是什么？','三步审查法：①受监管主体（电信/互联网/增值服务企业）②处理三类数据（个人信息/服务关系/用户生成数据）③被公安部A05认定涉及网络犯罪且未整改。'),
            ('违反越南数据本地化要求的后果是什么？','可处2-5亿越南盾罚款；A05可要求封锁网站/应用、暂停域名和广告服务。严重者导致企业在越南市场直接"消失"。'),
        ],
        'related': ['04-malaysia-pdpa','11-thailand-pdpa','14-romania-gdpr','15-vietnam-investment-law'],
    },
    '08_印尼IKN投资法律框架.md': {
        'slug': '08-indonesia-ikn', 'region': '印尼', 'tag': '投资法',
        'date': '2026-06-14', 'desc': 'IKN总投资预估300亿美元，OIKN享有超常规权力。解析外资准入的"超国民待遇"、土地征收法律争议与PPP模式的四种架构。',
        'faq': [
            ('外资在印尼IKN可以100%持股吗？','在建筑/数字基础设施/可再生能源/医疗/教育等领域，外资可在IKN区域内100%持股，享受"超国民待遇"。但战略国防和大众媒体仍受限。'),
            ('IKN的投资优惠有哪些？','建筑权(HGB)最长80年、耕作权(HGU)最长95年、OIKN一站式审批、特殊PPP合同模式、税收优惠等。'),
        ],
        'related': ['16-indonesia-omnibus','15-vietnam-investment-law','02-cambodia-qip'],
    },
    '09_泰国工厂法工业用地合规.md': {
        'slug': '09-thailand-factory-act', 'region': '泰国', 'tag': '行政合规',
        'date': '2026-06-15', 'desc': 'DIW 2026年启动3类工厂全面合规审查。详解从选址五维审查、EIA审批时间线到排污许可持续达标的工业用地全流程合规要点。',
        'faq': [
            ('泰国工厂选址需要审查哪些维度？','五维审查：城市规划(颜色区)、环境保护区、工业区(IEAT)局内局外、社区法定最小距离、交通红线与基础设施。'),
            ('泰国3类工厂的EIA审批需要多长时间？','从TOR提交到EIA批准，通常需12-18个月。建议同步推进EIA和BOI申请以节约项目前期时间。'),
        ],
        'related': ['01-thailand-tax-dispute','17-thailand-ma','11-thailand-pdpa'],
    },
    '10_东盟六国仲裁执行全景.md': {
        'slug': '10-asean-arbitration', 'region': '东盟', 'tag': '争议解决',
        'date': '2026-06-16', 'desc': '东盟六国《纽约公约》覆盖全景比较。从公共政策抗辩到正当程序抗辩，从SIAC到CIETAC的机构选择策略，附仲裁条款设计黄金模板。',
        'faq': [
            ('东盟国家中仲裁裁决执行最可靠的是哪个？','新加坡是区域首选，SIAC裁决在六国认可度最高。马来西亚和泰国法院对仲裁持相对支持态度。印尼和越南的执行不确定性较高。'),
            ('仲裁条款应该如何设计？','推荐SIAC为默认仲裁机构，仲裁地新加坡，适用SIAC规则。须加入紧急临时措施条款和放弃上诉权条款。关联合同应统一适用同一仲裁条款。'),
        ],
        'related': ['19-malaysia-competition','20-china-odi','17-thailand-ma'],
    },
    '11_泰国PDPA数据合规.md': {
        'slug': '11-thailand-pdpa', 'region': '泰国', 'tag': '数据合规',
        'date': '2026-06-17', 'desc': 'PDPC从制度建设期转入执法深化期，2025年罚款超1.2亿泰铢。详解五大高风险场景与PDPA/GDPR/PIPL三方合规要点。',
        'faq': [
            ('泰国PDPA与中国PIPL的主要区别是什么？','PDPA以"同意"为核心基础，暂未将"正当利益"列为独立合法性基础；PIPL规定了更严格的跨境传输安全评估制度；PDPA的独特性在于有监禁条款。'),
            ('中资企业在泰国最常见的PDPA违规是什么？','五大高风险场景：员工数据跨境回传中国、APP过度收集用户数据、客户数据用于跨境营销、视频监控与人脸识别、供应商数据共享缺少数据处理协议。'),
        ],
        'related': ['04-malaysia-pdpa','14-romania-gdpr','07-vietnam-data-localization'],
    },
    '12_柬埔寨劳动法实务.md': {
        'slug': '12-cambodia-labor-law', 'region': '柬埔寨', 'tag': '劳动法',
        'date': '2026-06-18', 'desc': '2022年《工会法》降至15人可建工会，欠薪追溯期翻倍至6个月。拆解雇佣、解雇、罢工与NSSF合规全流程。',
        'faq': [
            ('柬埔寨员工解雇需要支付多少补偿？','按工龄计算工龄补偿金(Seniority Indemnity)，每工作满一年支付15天工资。2022年修法将欠薪追溯期从3个月翻倍至6个月。'),
            ('柬埔寨工会的注册门槛是多少？','2022年新《工会法》规定，15名工人即可注册成立工会（原为7人），但需在劳动部注册登记。合法罢工须经15天通知+15天协商+7天投票的程序。'),
        ],
        'related': ['05-romania-labor-law','18-cambodia-sez','06-cambodia-construction'],
    },
    '13_马来西亚AMLA反洗钱合规.md': {
        'slug': '13-malaysia-amla', 'region': '马来西亚', 'tag': '刑事合规',
        'date': '2026-06-19', 'desc': '2024年AMLA修订引入公司刑事责任倒置，FATF灰名单压力推动执法升级。详解可疑交易报告、CDD义务与企业合规体系设计。',
        'faq': [
            ('马来西亚AMLA下公司刑事责任的标准是什么？','2024年修订引入责任倒置——公司须证明已建立"充分合规程序"方可免责。洗钱罪最高15年监禁。可疑交易须在交易发生后5个工作日内报告。'),
            ('中资企业在马来西亚哪些行为容易触发AMLA？','关联方转移定价、现金交易超过5万令吉、多层SPV境外架构、跨境大额资金转移无商业实质等均属高风险行为。'),
        ],
        'related': ['04-malaysia-pdpa','19-malaysia-competition','03-thailand-criminal-defense'],
    },
    '14_罗马尼亚GDPR数据保护.md': {
        'slug': '14-romania-gdpr', 'region': '罗马尼亚', 'tag': '数据合规',
        'date': '2026-06-20', 'desc': 'ANSPDCP累计处罚超800万欧元。对比GDPR/PIPL/PDPA三方差异，解析CCTV监控、员工数据跨境、电商Cookie四大高风险场景。',
        'faq': [
            ('罗马尼亚GDPR执法有多严？','ANSPDCP自2018年以来已作出450+项处罚决定，罚金总额超800万欧元。工厂CCTV违规和员工数据跨境传输是两大执法重点。'),
            ('中资企业在罗马尼亚如何合规处理员工数据？','须指定DPO、对摄像头进行CCTV告知、向中国传输员工数据须签署SCC或BCR、跨境传输前进行数据保护影响评估(DPIA)。'),
        ],
        'related': ['04-malaysia-pdpa','11-thailand-pdpa','05-romania-labor-law'],
    },
    '15_越南投资法外资准入.md': {
        'slug': '15-vietnam-investment-law', 'region': '越南', 'tag': '投资法',
        'date': '2026-06-21', 'desc': '2020年《投资法》实施负面清单制度，IRC与ERC双轨并行。解析外资持股上限、两步并购审批与代持安排无效化的最高法院判例。',
        'faq': [
            ('越南外资企业注册需要哪些证书？','双轨制：投资登记证书(IRC)+企业登记证书(ERC)。并购交易还需额外完成并购审批——涉及有条件行业的目标公司需要两步审批。'),
            ('越南哪些行业限制外资持股？','广告(不超过49%)、运输(不超过49%-51%)、电信(不超过49%-65%，但需外方为WTO成员国实体)、教育(不超过本地需求的一定比例)等行业均有上限。'),
        ],
        'related': ['07-vietnam-data-localization','08-indonesia-ikn','16-indonesia-omnibus'],
    },
    '16_印尼综合法Omnibus.md': {
        'slug': '16-indonesia-omnibus', 'region': '印尼', 'tag': '投资法',
        'date': '2026-06-22', 'desc': '2023年《创造就业综合法》重塑印尼投资生态。OSS-RBA简化许可、遣散费上限调整、环评简化、HGB延至80年——外资影响全解析。',
        'faq': [
            ('印尼Omnibus Law对外资最大的利好是什么？','通过OSS-RBA简化经营许可、外资建筑权(HGB)延长至80年、废除负面清单改为优先领域制度、劳动法遣散费从43倍上限降至25倍。'),
            ('Omnibus Law对劳工保护有何变化？','遣散费总额上限从43倍月薪降至25倍；固定期限合同从最长3年延至5年；外包限制放宽；最低工资公式与经济增长率挂钩。'),
        ],
        'related': ['08-indonesia-ikn','15-vietnam-investment-law','05-romania-labor-law'],
    },
    '17_泰国并购M&A实务.md': {
        'slug': '17-thailand-ma', 'region': '泰国', 'tag': '并购',
        'date': '2026-06-23', 'desc': '外商经营法限制如何影响交易架构？BOI/IEAT如何绕过FBA？10亿泰铢并购管制门槛、25%强制要约收购线——泰国M&A全流程指引。',
        'faq': [
            ('外资收购泰国公司最大的法律障碍是什么？','《外商经营法》(FBA)将多数行业列为清单三受限行业，外资持股须不高于49%。可通过BOI促进资格或IEAT工业区入驻来豁免FBA限制。'),
            ('泰国并购交易需要反垄断申报吗？','根据2017年《贸易竞争法》，并购后营业额超过10亿泰铢须向贸易竞争委员会申请事前审批。上市公司收购超过25%触发强制要约收购。'),
        ],
        'related': ['01-thailand-tax-dispute','09-thailand-factory-act','19-malaysia-competition'],
    },
    '18_柬埔寨经济特区SEZ.md': {
        'slug': '18-cambodia-sez', 'region': '柬埔寨', 'tag': '投资法',
        'date': '2026-06-24', 'desc': '24+个经济特区，SEZ内QIP享额外优惠。详解入驻流程、关税保税管理、劳工合规与西港特区中资工厂的14个月投产案例。',
        'faq': [
            ('柬埔寨经济特区(SEZ)相比区外有哪些优势？','进口设备/原材料免关税和增值税、增值税0%、简化海关手续、一站式行政服务、土地长期租赁权保障（50+25年）。'),
            ('柬埔寨有哪些主要的经济特区？','全国24+个SEZ，最受中资欢迎的是西哈努克港经济特区(SSEZ)、金边经济特区(PPSEZ)、大成经济特区等。SSEZ已有200+家企业入驻。'),
        ],
        'related': ['02-cambodia-qip','12-cambodia-labor-law','06-cambodia-construction'],
    },
    '19_马来西亚竞争法并购管制.md': {
        'slug': '19-malaysia-competition', 'region': '马来西亚', 'tag': '竞争法',
        'date': '2026-06-25', 'desc': '2025年马来西亚正式引入经营者集中申报制度。全球20亿+本地4000万令吉触发门槛，两阶段审查，罚金可达全球营业额10%。',
        'faq': [
            ('马来西亚经营者集中申报的触发门槛是什么？','全球合并营业额超20亿令吉，或其中一方在马来西亚营业额超4000万令吉、另一方在马来西亚营业额超4000万令吉或市场份额超20%。'),
            ('违反马来西亚并购管制的处罚是什么？','可处全球营业额的10%以下罚款、勒令拆分已完成交易、或处以每日罚款。Gun-jumping同样面临高额处罚。'),
        ],
        'related': ['17-thailand-ma','13-malaysia-amla','10-asean-arbitration'],
    },
    '20_中国ODI备案全流程.md': {
        'slug': '20-china-odi', 'region': '跨境', 'tag': 'ODI',
        'date': '2026-06-26', 'desc': '发改委11号令+商务部3号令+外汇登记，三轨并行。ODI备案、核准、报告全流程实操指引。',
        'faq': [
            ('ODI备案需要多长时间？','非敏感、投资额3亿美元以下项目：发改委备案7-20个工作日+商务部备案3-20个工作日+外汇登记约5个工作日。核准项目（敏感或超3亿美元）时间更长。'),
            ('哪些行业属于ODI敏感行业？','房地产、酒店、影城、娱乐业、体育俱乐部、枪支弹药、武器装备、跨境水资源开发、新闻传媒等。这些行业的ODI需核准而非备案。'),
        ],
        'related': ['02-cambodia-qip','15-vietnam-investment-law','08-indonesia-ikn'],
    },
    '21_柬埔寨税务制度合规.md': {
        'slug': '21-cambodia-tax', 'region': '柬埔寨', 'tag': '税务合规',
        'date': '2026-06-27', 'desc': '柬埔寨五大税种详解：利润税、预扣税、增值税、最低税、薪资税。拆解QIP税收优惠、转让定价合规、税务稽查应对与GDT数字化转型。',
        'faq': [('柬埔寨企业所得税税率是多少？','标准税率20%，石油/天然气和特定自然资源开采行业为30%。QIP项目可享受3-9年免税期，免税期届满后可适用较低的优惠税率。'),
                ('柬埔寨QIP免税期届满后适用什么税率？','QIP免税期届满后，按行业和投资活动分级适用加速税率表。最低税(1%年营业额)在免税期内豁免，免税期届满后恢复适用。'),
                ('柬埔寨税务稽查如何应对？','收到稽查通知后应立即委托当地税务律师，整理近3年的完整账簿和凭证，配合稽查官员的合法要求。切勿隐匿财务资料或转移公司资产——这在柬埔寨是刑事犯罪。')],
        'related': ['02-cambodia-qip','18-cambodia-sez','22-cambodia-land'],
    },
    '22_柬埔寨土地法外资路径.md': {
        'slug': '22-cambodia-land', 'region': '柬埔寨', 'tag': '土地法',
        'date': '2026-06-28', 'desc': '宪法第44条禁止外国人持有土地所有权。详解四种合法路径：长期租赁、经济土地特许、共管公寓、土地控股公司，以及代持陷阱的致命风险。',
        'faq': [('外国人能在柬埔寨购买土地吗？','不能。柬埔寨宪法第44条禁止外国人持有土地所有权。但外国人可通过长期租赁(50+25年)、经济土地特许、购买共管公寓一层以上单元等合法方式取得土地使用权。'),
                ('通过柬埔寨人代持土地有什么风险？','代持是无效且违法的手段。常见风险包括：代持人私下出售土地、代持人死亡继承人拒绝返还、柬政府打击代持时土地被没收。2019年后柬埔寨严打代持安排，涉案土地可能被冻结。')],
        'related': ['02-cambodia-qip','18-cambodia-sez','06-cambodia-construction'],
    },
    '23_柬埔寨知识产权保护.md': {
        'slug': '23-cambodia-ip', 'region': '柬埔寨', 'tag': '知识产权',
        'date': '2026-06-29', 'desc': '先申请原则下的商标抢注风险、专利审查的实务难点、边境执法的全新利器。蜜雪冰城商标抢注案深度复盘，中资品牌入柬前必修IP课。',
        'faq': [('柬埔寨商标注册采用什么原则？','先申请原则(First-to-File)。商标权授予最先提交申请的一方。在柬埔寨没有使用的商标同样可被注册——这使得"抢注"成为中资品牌入柬最常见的IP陷阱。'),
                ('中国商标在柬埔寨受保护吗？','中国商标注册在柬埔寨不自动受保护。需要通过马德里议定书指定柬埔寨或直接在柬埔寨商业部知识产权局(DIP)提交国家注册申请。')],
        'related': ['21-cambodia-tax','26-cambodia-digital','18-cambodia-sez'],
    },
    '24_柬埔寨外汇管理资金进出.md': {
        'slug': '24-cambodia-fx', 'region': '柬埔寨', 'tag': '外汇管理',
        'date': '2026-06-30', 'desc': '高度自由的柬埔寨外汇体制下暗藏合规玄机。利润汇回五步流程、交叉货币风险管控、NBC反洗钱报告义务、地下钱庄打击行动——中资企业资金管理的全流程指引。',
        'faq': [('柬埔寨有外汇管制吗？','柬埔寨实行高度自由化的外汇管理体制，无资本管制措施。根据1997年《外汇法》，利润、股息、资本利得、特许权使用费等均可自由汇出。但单笔大额汇款须向柬埔寨国家银行(NBC)报告。'),
                ('从柬埔寨向中国汇回利润需要什么手续？','标准流程：税务清算证明(GDT)→董事会分红决议→审计报告→银行汇款申请→中国外汇管理局报告。注意：必须先完成柬埔寨的税务清算，否则银行拒绝处理。')],
        'related': ['21-cambodia-tax','20-china-odi','02-cambodia-qip'],
    },
    '25_柬埔寨环境法环评合规.md': {
        'slug': '25-cambodia-environment', 'region': '柬埔寨', 'tag': '环境法',
        'date': '2026-07-01', 'desc': '2025年Prakas 4828重塑EIA分类制度。详解EIA审批流程、环境合约履约体系、碳信用机遇与中国工厂的环保红线——从$50,000罚款案说起。',
        'faq': [('柬埔寨哪些项目需要做EIA？','工业采矿、大型制造业工厂、经济特区(SEZ)、电力项目(≥10MW)、大型基础设施(公路/港口/机场)、涉及自然保护区的开发项目等。2025年Prakas 4828对小型矿业项目提供了1公顷以下的环评豁免。'),
                ('柬埔寨EIA审批需要多长时间？','从TOR提交到EIA批准，整体周期约12-18个月。TOR审批45-60个工作日，EIA报告编制3-6个月，专家委员会审批60-90个工作日。环评未获批前不得启动任何现场施工。')],
        'related': ['02-cambodia-qip','18-cambodia-sez','06-cambodia-construction'],
    },
    '26_柬埔寨数字经济电商法.md': {
        'slug': '26-cambodia-digital', 'region': '柬埔寨', 'tag': '数字经济',
        'date': '2026-07-02', 'desc': '130%移动渗透率+40%电商增速+监管真空期——柬埔寨数字经济的机遇窗口。电商法、支付监管、网络安全法草案、数据保护法展望，中资科技公司的进入策略全解析。',
        'faq': [('中国科技企业在柬埔寨开展电商业务需要什么许可？','依据2019年《电子商务法》和《消费者保护法》，在柬埔寨运营电商平台须向商务部注册、遵守七天无理由退货规则(消费者保护法第12条)、在网页显著位置公示经营主体信息和营业许可。外资可在柬埔寨100%持股科技公司。'),
                ('柬埔寨有个人数据保护法吗？','目前尚未颁布综合性的个人数据保护法。2026年，柬埔寨邮电部和商务部正在联合起草《个人数据保护法》草案，预计2026-2027年提交国会审议。企业当前应参照东盟数据保护框架和即将出台的法律草案进行合规准备。')],
        'related': ['04-malaysia-pdpa','07-vietnam-data-localization','11-thailand-pdpa'],
    },
    '27_柬埔寨破产法债权人保护.md': {
        'slug': '27-cambodia-insolvency', 'region': '柬埔寨', 'tag': '破产法',
        'date': '2026-07-03', 'desc': '2019年新《破产法》取代旧法，债权人权利显著增强。法定优先顺序、可撤销交易追溯期、破产申请作为谈判筹码——中资企业追讨债务的全套法律武器。',
        'faq': [('柬埔寨破产清偿的顺序是什么？','第一顺位：担保债权人(以其担保物价值为限)；第二顺位：破产管理费用(含司法管理官报酬)；第三顺位：欠付员工工资(超级优先——6个月上限)；第四顺位：税务机关税款；第五顺位：无担保债权人；最后：股东。'),
                ('债务人进入破产程序后，债权人还能追回资产吗？','可以。2019年《破产法》赋予司法管理官"撤销权"——法院可裁定撤销债务人在破产前特定期限内进行的低价交易(2年内)、偏袒性支付(6个月内)和欺诈性财产转移(无时间限制)。')],
        'related': ['24-cambodia-fx','21-cambodia-tax','10-asean-arbitration'],
    },
    '28_泰国破产重整企业拯救.md': {
        'slug': '28-thailand-bankruptcy', 'region': '泰国', 'tag': '破产重整',
        'date': '2026-07-04', 'desc': '泰国2016年新增SME简易重整、2018年采纳UNCITRAL跨国界破产示范法。重整计划如何制定、债权人如何保护自己、境内财产如何不被跨境转移——中资企业债务危机的法律应对全解析。',
        'faq': [('泰国破产重整程序的启动条件是什么？','债权人可申请：个人债务人欠款≥100万泰铢、企业债务人欠款≥200万泰铢。债务人可自行申请重整：债务总额≥1,000万泰铢(有限公司)或≥200万泰铢(SME)。法院受理后产生自动中止效力——所有债权追索程序立即暂停。'),
                ('在泰国，债权人可以申请债务人破产吗？','可以。只要债权总额≥200万泰铢(企业)或≥100万泰铢(个人)，债务人到期未偿还且在债权人催告后30日内未能支付或提供合理担保的，债权人即可向破产法院提起破产申请。')],
        'related': ['01-thailand-tax-dispute','17-thailand-ma','27-cambodia-insolvency'],
    },
    '29_越南劳动法外籍员工.md': {
        'slug': '29-vietnam-labor', 'region': '越南', 'tag': '劳动法',
        'date': '2026-07-05', 'desc': '30天提前公开招聘义务、配额制度下的外籍员工数量限制、2022年起的强制社保——2019年《劳动法》对中方派遣人员的三重管控，及2023年第70号令的最新调整。',
        'faq': [('中国员工在越南工作需要什么许可？','越南雇主须为外籍员工申请工作许可(Work Permit)。申请前提：该岗位已向越南本地求职者公开招聘至少30天仍无法招募到合适人选。工作许可有效期最长2年，可续期一次。免办工作许可的情形极为有限(如持股超过一定比例的出资人、董事会成员等)。'),
                ('违反越南外籍劳工规定有什么后果？','外籍员工个人：可被驱逐出境并列入入境黑名单。雇主企业：行政罚款、6个月至1年内不得从非越南籍国家招聘新劳工、在越南投资登记系统中的雇主评级降级(影响未来投资审批)。')],
        'related': ['12-cambodia-labor-law','05-romania-labor-law','15-vietnam-investment-law'],
    },
    '30_一带一路投资风险地图.md': {
        'slug': '30-bri-risk-map', 'region': '跨境', 'tag': '投资法',
        'date': '2026-07-06', 'desc': '六国（泰柬马越印尼罗马尼亚）九大法律维度横向对比表，+政治风险维度共十项尽调清单。从外资准入到争议解决，从税务到劳动，中资出海前的全维度法律风险评估框架。',
        'faq': [('中资企业出海投资前应该做哪些法律尽调？','十项核心尽调清单：①外资准入限制及国家安全审查机制 ②土地所有权和使用权 ③税收制度及投资激励 ④劳动法及外籍员工管理 ⑤环境影响评价(EIA)要求 ⑥外汇管制及利润汇回制度 ⑦争议解决机制(法院vs仲裁) ⑧知识产权保护水平 ⑨反腐败及刑事合规 ⑩政治风险及投资条约保护。'),
                ('东盟国家中哪些投资风险最高？','从法律不确定性角度：柬埔寨(法律体系仍在发展中)、印尼(政策频繁调整)、越南(法规执行标准不透明)。从政治风险角度：各国均处于中低水平，但需关注特定行业(如柬埔寨博彩业、印尼矿业)的政策突转风险。')],
        'related': ['20-china-odi','10-asean-arbitration','08-indonesia-ikn'],
    },
    '31_泰国工作签证与社保合规.md': {
        'slug': '31-thailand-work-permit', 'region': '泰国', 'tag': '劳动合规',
        'date': '2026-07-07', 'desc': '2025年查处外籍劳工违规案件2,847起，中籍员工违规437起增长28%。详解Non-B签证、工作许可、社保合规、BOI/IEAT绿色通道与突击检查应对策略。',
        'faq': [
            ('中国人在泰国工作需要什么签证和许可？','须持Non-B商务签证入境（90天），入境后30天内办理工作许可(Work Permit)。BOI企业可走快速通道3-5个工作日获批，SMART Visa持有者免办工作许可。'),
            ('泰国社保对外籍员工是强制性的吗？','是。2023年起所有在泰国合法工作的外国人（含工作许可持有者）均须参加泰国社会保障体系，不能以"已在中国缴纳五险一金"为由豁免。雇主承担5%+雇员5%。'),
            ('在泰国违反工作许可规定有什么后果？','个人：5,000-50,000泰铢罚款+驱逐出境+最长10年黑名单。雇主：10,000-100,000泰铢罚款，BOI企业可能被暂停/撤销优惠。'),
        ],
        'related': ['01-thailand-tax-dispute','09-thailand-factory-act','17-thailand-ma'],
    },
    '32_柬埔寨银行与金融监管.md': {
        'slug': '32-cambodia-banking', 'region': '柬埔寨', 'tag': '金融监管',
        'date': '2026-07-08', 'desc': 'NBC 2026年启动金融科技沙盒2.0，资本充足率提至15.5%。拆解四类银行牌照、MFI/PSI申请路径、CAFIU反洗钱执法趋势与中资金融机构的进入策略。',
        'faq': [
            ('外资可以在柬埔寨100%持股银行吗？','可以。柬埔寨银行体系对外资持完全开放态度，外资可100%持有柬埔寨银行牌照。但NBC要求母行评级不低于BBB(BBB/S&P)或Baa2(穆迪)，且须取得母国监管确认函。'),
            ('柬埔寨商业银行的最低资本要求是多少？','全能银行4000亿瑞尔(≈1亿美元)、商业银行1500亿瑞尔(≈3,750万美元)、专业银行300亿瑞尔(≈750万美元)、外资银行分行须拨入1亿美元运营资金。'),
        ],
        'related': ['21-cambodia-tax','24-cambodia-fx','26-cambodia-digital'],
    },
    '33_马来西亚劳动法外籍员工.md': {
        'slug': '33-malaysia-employment', 'region': '马来西亚', 'tag': '劳动法',
        'date': '2026-07-09', 'desc': '2026年《雇佣法》全面保护月薪4,000令吉以下员工。详解外籍员工四道准入审批、解雇正当理由制度、国内调查(DI)程序与EPF/SOCSO合规要点。',
        'faq': [
            ('马来西亚解雇员工需要什么程序？','须有"正当理由"(Just Cause)。因不当行为解雇须经国内调查(Domestic Inquiry)程序：发出Show Cause Letter→组建调查小组→听证会→作出结论。程序瑕疵可导致工业法院裁定解雇无效。'),
            ('中国员工在马来西亚工作须参加当地社保吗？','是。须参加EPF(公积金)和SOCSO(社会保障)。2024年起外籍员工可选择较低EPF费率(9%+11%)。中马尚无社会保障双边协议——意味着双重缴费风险。'),
        ],
        'related': ['12-cambodia-labor-law','05-romania-labor-law','29-vietnam-labor'],
    },
    '34_罗马尼亚公司法与外商投资.md': {
        'slug': '34-romania-company-law', 'region': '罗马尼亚', 'tag': '公司法',
        'date': '2026-07-10', 'desc': '2026年第32号法律引入实际受益人电子申报和CEISD审查扩展。拆解SRL/SA/分支机构设立全流程、外商投资安全审查触发条件与欧盟公司法的多层约束。',
        'faq': [
            ('外国人在罗马尼亚设立公司的最低注册资本是多少？','有限责任公司(SRL)最低注册资本为1列伊(≈0.2欧元)，但实践中建议注资不低于10,000欧元以维持商业信誉。股份公司(SA)最低注册资本90,000列伊(≈18,000欧元)。'),
            ('在罗马尼亚投资需要外商投资审查吗？','欧盟外投资者在罗投资超过200万欧元且涉及关键基础设施、关键技术、关键资源、敏感数据、国防、健康等行业的，须经CEISD审查。审查周期60个工作日。"抢跑"可导致交易无效+投资额10%罚款。'),
        ],
        'related': ['05-romania-labor-law','14-romania-gdpr','15-vietnam-investment-law'],
    },
    '35_越南税务与转让定价.md': {
        'slug': '35-vietnam-tax', 'region': '越南', 'tag': '税务合规',
        'date': '2026-07-11', 'desc': '2025年GDT转让定价稽查追缴16.8万亿越南盾（占追缴总额40%）。详解CIT/VAT/FCWT三大税种、三层转让定价文档体系、全球最低税冲击与税务争议应对。',
        'faq': [
            ('越南企业所得税的标准税率和优惠是什么？','标准税率20%。高科技/大规模制造可享受10%(15年)、15%(10年)等优惠税率，叠加"4免9减半"或"2免4减半"免税期。但全球最低税(GloBE)可能将有效税率补足至15%。'),
            ('越南转让定价稽查的重点是什么？','六大重点：持续亏损但仍扩大投资的FIE、利润率显著低于行业平均、向关联方支付高额服务费/许可费、无形资产转让、多层导管跨境交易、关联贷款高息或免息。未能按时提交转让定价文档的，GDT可自行"推定"交易价格。'),
        ],
        'related': ['01-thailand-tax-dispute','21-cambodia-tax','15-vietnam-investment-law'],
    },
    '36_印尼个人数据保护法.md': {
        'slug': '36-indonesia-pdp', 'region': '印尼', 'tag': '数据合规',
        'date': '2026-07-12', 'desc': '印尼UU PDP 2026年执法过渡期届满，LPDP启动首批执法行动。解析域外管辖、6项合法性基础、DPO强制任命、3×24小时泄露通知与年营收2%罚款——中资科技企业的数据合规新挑战。',
        'faq': [
            ('印尼PDP法有域外管辖效力吗？','有。UU PDP适用于：①在印尼境内处理数据；②在境外处理但产生印尼境内法律效果；③涉及印尼公民个人数据——无论数据处理者是否在印尼设有机构。'),
            ('印尼PDP的处罚有多严厉？','行政处罚最高达年营收的2%。刑事制裁最高6年监禁+60亿盾罚款（法人罚金可达个人3倍）。公司可因未尽防止犯罪义务被追究刑事责任（Superior Responsibility Doctrine）。'),
        ],
        'related': ['11-thailand-pdpa','04-malaysia-pdpa','07-vietnam-data-localization'],
    },
    '37_泰国公寓法与外国人购房.md': {
        'slug': '37-thailand-condominium', 'region': '泰国', 'tag': '房地产法',
        'date': '2026-07-13', 'desc': '中国买家以46.6%占比居泰国外籍购房者首位。详解49%外国人配额、FET Form资金汇入要求、代持致命风险、开发商违约救济与替代性持有结构的法律评估。',
        'faq': [
            ('外国人在泰国购买公寓有什么限制？','可合法持有公寓永久产权(Freehold)，但不得超过该公寓项目总出售面积的49%（"外国人配额"）。购房资金须从境外汇入泰国，并取得银行FET Form。土地绝对禁止外国人持有。'),
            ('泰国公寓"外国人配额"满了怎么办？','不可通过泰籍人士代持——代持属违法，可致交易无效+刑事追责+移民黑名单。替代方案：①30年租赁权(Leasehold)；②设立泰国公司(外资≤49%)以公司名义购买；③等待配额空出（其他外国业主出售）。'),
        ],
        'related': ['03-thailand-criminal-defense','11-thailand-pdpa','17-thailand-ma'],
    },
    '38_东盟跨境电商法律合规.md': {
        'slug': '38-asean-ecommerce', 'region': '东盟', 'tag': '数字经济',
        'date': '2026-07-14', 'desc': '东盟电商GMV达1,860亿美元，多国监管集中"收网"。六维度拆解：市场准入、海关税务（LVG/VAT代扣）、消费者保护、数据合规、知识产权、争议解决——中资电商平台的合规行动清单。',
        'faq': [
            ('印尼对跨境电商有什么特殊限制？','最严：社交媒体与电商分离(Permendag 31/2023)、跨境最低货值100美元、正面清单制度（禁止廉价纺织品/鞋类/电子产品跨境销售）。外资电商须设立PT PMA。'),
            ('东盟国家中哪些电商市场最开放？','从监管宽松度排序：柬埔寨>马来西亚>菲律宾>泰国>越南>印尼。柬埔寨电商监管仍处于早期阶段，是当前合规成本最低的窗口期市场。'),
        ],
        'related': ['07-vietnam-data-localization','11-thailand-pdpa','26-cambodia-digital'],
    },
    '39_FIDIC国际工程合同.md': {
        'slug': '39-fidic-contracts', 'region': '跨境', 'tag': '工程法',
        'date': '2026-07-15', 'desc': '东盟中资工程80%采用FIDIC合同。深度拆解十大攻防要害：Time Bar（28天索赔时效灭失）、工程师决定、不可预见条件、DLD误期赔偿、DAAB裁决约束力——程序即权利。',
        'faq': [
            ('FIDIC合同下索赔的"Time Bar"是什么意思？','承包商须在知晓索赔事件后28天内向工程师发出索赔通知，否则丧失索赔权利。2017版还要求在84天内提交"详细充分索赔"。这是FIDIC对承包商最无情的条款——"先干活后算账"的中国管理文化在此致命。'),
            ('FIDIC合同下的"工程师"是公平的吗？','理论上工程师应"中立"(2017版措辞从"公正"升级为"中立")，但实践中工程师由雇主聘任并支付报酬，天然倾向雇主利益。承包商须对所有口头指令进行书面确认(CVI)，所有决定不满须在时限内发出"异议通知"(NOD)——保留后续仲裁权利。'),
        ],
        'related': ['10-asean-arbitration','06-cambodia-construction','30-bri-risk-map'],
    },
    '40_跨境税务筹划与双边税收协定.md': {
        'slug': '40-cross-border-tax', 'region': '跨境', 'tag': '税务筹划',
        'date': '2026-07-16', 'desc': '约40%中资企业未充分利用DTA优惠。详解中国-东盟DTA优惠税率、控股架构税务逻辑、资本弱化规则、关联方贷款转让定价，以及全球最低税(GloBE)对传统税务筹划的颠覆。',
        'faq': [
            ('中国与东盟国家之间的股息预提税率是多少？','优惠税率通常为5%-10%（取决于持股比例和受益所有人资格）。如中国-新加坡为5%(持股≥25%)、中国-马来西亚为5%(持股≥25%)、中国-越南为5%(持股≥70%)。须满足"受益所有人"实质要求。'),
            ('全球最低税(GloBE)对中资海外投资有什么影响？','合并年营收超7.5亿欧元的跨国企业——海外子公司在低税率辖区的利润将被征收"补充税"补足至15%。传统"低税天堂中间控股"、"利息扣除最大化"、"无形财产利润转移"三大策略均受到根本性挑战。中等规模企业暂有几个年的过渡窗口期。'),
        ],
        'related': ['20-china-odi','21-cambodia-tax','35-vietnam-tax'],
    },
    '41_马来西亚税务制度与投资激励.md': {
        'slug': '41-malaysia-tax', 'region': '马来西亚', 'tag': '税务合规',
        'date': '2026-07-19', 'desc': '2025年RMCD转让定价稽查占追缴额36%。拆解24%企业所得税、SST 2.0、预提税体系、Pioneer Status/ITA/RA三重激励，以及三层转让定价文档的合规要求。',
        'faq': [
            ('马来西亚企业所得税税率是多少？','标准税率24%，中小企业首60万令吉适用15%优惠税率。马来西亚实行地域来源课税制——海外来源收入在满足实质条件下免税。2024年起对非上市公司股份处置征收10%资本利得税。'),
            ('外国企业在马来西亚可以享受哪些税收优惠？','核心激励工具：Pioneer Status（5-10年免70%所得税）、Investment Tax Allowance（合格资本支出60%-100%抵扣）、Reinvestment Allowance（制造业再投资60%抵扣15年）。数字/生物科技/航空航天等行业有额外专项激励。'),
        ],
        'related': ['13-malaysia-amla','19-malaysia-competition','33-malaysia-employment'],
    },
    '42_马来西亚外资准入与市场进入.md': {
        'slug': '42-malaysia-foreign-investment', 'region': '马来西亚', 'tag': '投资法',
        'date': '2026-07-20', 'desc': '马来西亚外资准入"隐性监管"——30余部行业法分散规定外资持股上限。拆解制造业/服务业分行业限制、公司设立流程、四种市场进入策略及NIA 2024-2030国家投资愿景的影响。',
        'faq': [
            ('外资在马来西亚制造业可以100%持股吗？','原则上可以。马来西亚自2003年已取消制造业外资持股限制，仅少数子行业（中小型包装材料70%、塑料废物回收等）有限制。但低附加值劳动密集型项目在NIA新框架下越来越难获得MIDA批准。'),
            ('马来西亚服务业的外资持股限制有哪些？','金融（银行30%、保险70%）、电信（网络设施49%、应用服务70%）、物流（货运代理49%、港口49%）、零售（须取WRT准证+100万令吉实缴资本）、教育（K-12≤49%、大学100%）等行业存在差异化外资上限。'),
        ],
        'related': ['19-malaysia-competition','41-malaysia-tax','04-malaysia-pdpa'],
    },
    '43_马来西亚外资房地产法律实务.md': {
        'slug': '43-malaysia-property', 'region': '马来西亚', 'tag': '房地产法',
        'date': '2026-07-21', 'desc': '中国买家连续五年居马国外资购房榜首。详解各州最低价格门槛（100万-200万令吉）、州政府同意函制度、马来保留地的绝对排他性、MM2H计划购房通道，以及Bumiputera配额对外资开发商的约束。',
        'faq': [
            ('外国人在马来西亚买房的最低价格门槛是多少？','吉隆坡100万令吉、雪兰莪100万-200万令吉（分三区）、柔佛100万-200万令吉、槟城100万（大陆）/200万（槟岛）。须取得州政府"外国人购房同意函"（审批2-6个月），马来保留地和Bumiputera Lot绝对禁止外国人购买。'),
            ('马来西亚第二家园(MM2H)对购房有什么优势？','2024年新MM2H分三类（银/黄金/白金），定存要求50万-500万令吉。MM2H持有者可购买低于州政府外国人最低价格门槛的住宅（柔佛/槟城适用较低标准），且州政府同意函审批相对宽松。'),
        ],
        'related': ['37-thailand-condominium','22-cambodia-land','42-malaysia-foreign-investment'],
    },
    '44_罗马尼亚税务制度与欧盟资金申请.md': {
        'slug': '44-romania-tax', 'region': '罗马尼亚', 'tag': '税务合规',
        'date': '2026-07-22', 'desc': '2025年ANAF转让定价稽查追缴3.5亿欧元增长43%。拆解16%企业所得税、19% VAT、中罗DTA 3%优惠预提税、RO e-Factura电子发票系统，以及中资企业申请欧盟结构性基金和PNRR的合规路径。',
        'faq': [
            ('罗马尼亚企业所得税税率是多少？','标准税率16%，是欧盟最低之一。微型企业（年营收≤50万欧元）适用1%-3%按营收征税制。中罗DTA提供3%的股息/利息/特许权使用费预提税率，是中国签署的最优DTA之一。2025年起全面实施RO e-Factura电子发票制度。'),
            ('在罗马尼亚的中资企业能申请欧盟资金吗？','可以。以罗马尼亚子公司（SRL/SA）身份申请。欧盟2021-2027年预算期对罗拨付约310亿欧元凝聚基金+292亿欧元PNRR复苏韧性基金。中资须证明子公司在罗具有"实质性经营"，部分项目需与欧盟本地企业组成联合体。'),
        ],
        'related': ['34-romania-company-law','14-romania-gdpr','05-romania-labor-law'],
    },
    '45_罗马尼亚可再生能源投资法律实务.md': {
        'slug': '45-romania-renewable-energy', 'region': '罗马尼亚', 'tag': '能源法',
        'date': '2026-07-23', 'desc': '2025年罗新增光伏2.1GW，中资占11%。拆解绿色证书→CfD制度转型、48个月开发全流程、弃电与并网延迟风险、中罗投资架构设计，以及CEISD外商投资审查对可再生能源项目的特殊影响。',
        'faq': [
            ('罗马尼亚可再生能源项目的开发周期有多长？','光伏项目从土地获取到商业运营约24-42个月，风电约30-54个月。最耗时节点：ATR并网审批（6-18个月）+建筑许可（6-12个月）+施工（光伏8-14月/风电12-24月）。2025年ANRE取消2MW以上项目自动并网许可，进一步延长了审批周期。'),
            ('罗马尼亚绿色证书制度发生了什么变化？','原绿色证书(CV)制度从2017年起多次削减（光伏从6 CV/MWh降至约3 CV）。2026年起对5MW以上新建项目引入CfD（差价合约）拍卖机制——15年期固定电价保证，取代CV制度。CfD提供更高的政策确定性，但拍卖竞争激烈程度不确定。'),
        ],
        'related': ['34-romania-company-law','44-romania-tax','39-fidic-contracts'],
    },
    '46_罗马尼亚争议解决与仲裁.md': {
        'slug': '46-romania-dispute', 'region': '罗马尼亚', 'tag': '争议解决',
        'date': '2026-07-24', 'desc': 'CICA-CCIR 2025年受理314件仲裁案涉中资企业21件增长36%。拆解法院四级审级、紧急仲裁员48小时裁定、欧盟EAPO账户冻结令、跨境判决裁决执行路径与中资企业维权策略。',
        'faq': [
            ('在罗马尼亚打官司，外国原告需要特别注意什么？','三个关键点：诉讼费用担保义务（非欧盟原告须提供不超争议金额10%的担保）、送达地址（须在罗指定送达代收人）、翻译义务（所有非罗语文件须附经认证的罗语翻译）。'),
            ('罗马尼亚仲裁裁决在中国法院执行的成功率如何？','两国均为纽约公约缔约国。CICA-CCIR裁决在中国的执行先例较少，建议选择SIAC或ICC仲裁——两家机构裁决在中国执行记录更丰富、可预期性更高。'),
            ('在罗马尼亚可以申请"诉前财产保全"吗？','可以。须同时满足：紧急情况+初步证据+保全担保（10%-20%保全金额）。法院3-10个工作日内裁定。银行账户冻结最快最有效——裁定送达银行后即时生效。'),
        ],
        'related': ['34-romania-company-law','44-romania-tax','10-asean-arbitration'],
    },
    '47_罗马尼亚外资安全审查深度解析.md': {
        'slug': '47-romania-fdi', 'region': '罗马尼亚', 'tag': '外资审查',
        'date': '2026-07-25', 'desc': '2026年第32号法律将CEISD审查股权触发门槛从25%降至10%，扩展至数据中心和AI领域。拆解三层触发机制、两阶段审查流程、抢跑最高投资额10%罚款与欧盟FDI筛选网络的叠加效应。',
        'faq': [
            ('什么样的投资100%会触发CEISD审查？','同时满足：非欧盟投资者+收购罗企业10%以上股权或获实际控制权+目标企业属关键行业（关键基础设施/关键技术/敏感数据/国防/媒体/健康）。可再生能源(≥50MW)、数据中心、半导体、5G通信最常触发审查。'),
            ('通过新加坡/香港子公司收购能规避CEISD审查吗？','不能。CEISD自2023年GEO 257/2023起实行穿透式审查——审查追溯至最终实际受益人(UBO)。即使收购主体是新加坡/香港子公司，如UBO仍为中国公民/实体，仍按非欧盟投资者标准审查。'),
        ],
        'related': ['34-romania-company-law','45-romania-renewable-energy','44-romania-tax'],
    },
    '48_罗马尼亚房地产与建筑工程.md': {
        'slug': '48-romania-realestate', 'region': '罗马尼亚', 'tag': '房地产法',
        'date': '2026-07-26', 'desc': '布加勒斯特核心区公寓突破2,500欧元/㎡。拆解外国人土地持有限制、共管公寓购买程序、CU至AC的12-24个月开发许可全流程、FIDIC合同的罗马尼亚法院本地化解释规则与中资开发商五查尽调清单。',
        'faq': [
            ('中国人个人能在罗马尼亚买房吗？','可自由购买共管公寓（永久产权），但不可直接购买土地。土地须通过在罗设立的实际经营子公司持有。购房资金须从中国境内银行以"境外购房"名义合法汇出。'),
            ('从签署预购合同到拿到钥匙需要多长时间？','新房（期房）：12-24个月。二手房：30-60天——核心环节是CF产权核查（5-15天）+公证人准备合同（5-10天）+CF登记（10-20天）。'),
        ],
        'related': ['34-romania-company-law','05-romania-labor-law','39-fidic-contracts'],
    },
    '49_马来西亚争议解决与仲裁.md': {
        'slug': '49-malaysia-dispute', 'region': '马来西亚', 'tag': '争议解决',
        'date': '2026-07-27', 'desc': 'AIAC 2025年受理187件仲裁案涉中资29件。拆解HCM商事庭快速审判、即决判决机制、CIPAA 45天建筑业快速裁决、Mareva禁制令数小时内冻结资产与SIAC/AIAC仲裁条款设计策略。',
        'faq': [
            ('马来西亚法院诉讼和仲裁，哪个更适合中资企业？','仲裁。仲裁裁决在《纽约公约》框架下跨境执行通畅，法院判决的跨境执行存在显著障碍。仲裁保密性强、一裁终局。法院诉讼的优势场景是：需Mareva禁制令或即决判决快速追偿。'),
            ('马来西亚仲裁裁决在中国执行的成功率如何？','AIAC/SIAC裁决在中国法院获得承认执行的总体成功率约80%-85%。主要失败原因：仲裁协议无效、仲裁通知未适当送达。选择SIAC仲裁时受益于中新双边司法协作的良好基础。'),
        ],
        'related': ['10-asean-arbitration','19-malaysia-competition','42-malaysia-foreign-investment'],
    },
    '50_马来西亚知识产权保护.md': {
        'slug': '50-malaysia-ip', 'region': '马来西亚', 'tag': '知识产权',
        'date': '2026-07-28', 'desc': '中资企业马来西亚商标申请量居外国首位（5,872件），但抢注案件同比增长47%。拆解先申请原则下的攻防体系、ASPEC/PPH专利加速审查、海关边境措施与品牌出海反抢注的六步实战策略。',
        'faq': [
            ('中国注册商标在马来西亚受保护吗？','不自动保护。须通过马德里体系指定马来西亚或直接在MyIPO提交国家申请。驰名商标虽有巴黎公约反抢注保护但门槛极高，建议直接注册而非依赖驰名商标保护。'),
            ('马来西亚实用新型和发明专利如何选择？','渐进式改进（设备优化、流程改进）→申请实用新型（审查12-18个月）。突破性创新（全新化合物/药物/核心算法）→申请发明专利（审查3-5年）。中资制造企业通常双轨并行——核心产品申请发明专利，外围改进申请实用新型。'),
        ],
        'related': ['23-cambodia-ip','04-malaysia-pdpa','42-malaysia-foreign-investment'],
    },
    '51_马来西亚建筑工程法律实务.md': {
        'slug': '51-malaysia-construction', 'region': '马来西亚', 'tag': '工程法',
        'date': '2026-07-29', 'desc': 'CIDB注册G7外资承包商420家中资占38%。拆解CIDB七级注册体系、PWD/PAM/FIDIC合同选择策略、CIPAA 45天建筑业快速裁决机制、CCC完工合规证书制度与中资承包商劳动力/环保红线。',
        'faq': [
            ('CIPAA裁决和普通仲裁有什么区别？','CIPAA是马来西亚特有的建筑业法定裁决机制——速度极快（45个工作日内裁决）、成本低（3,000-15,000令吉）、裁决具临时约束力（业主须先行支付再通过仲裁"重新决定"）。普通仲裁需12-18个月。对中资承包商而言CIPAA是追讨工程款的首选工具。'),
            ('马来西亚CIDB的G7注册有什么要求？','最低实缴资本750,000令吉+至少2名本地持牌技术负责人+最近3年累计完成建筑工程不低于5,000万令吉+ISO 9001认证+无破产/清算记录。申请周期通常3-6个月。'),
        ],
        'related': ['06-cambodia-construction','39-fidic-contracts','10-asean-arbitration'],
    },
    '52_越南争议解决与仲裁.md': {
        'slug': '52-vietnam-dispute', 'region': '越南', 'tag': '争议解决',
        'date': '2026-07-30', 'desc': 'VIAC 2025年受理412件仲裁案涉中资78件占19%，中资是VIAC最大外国用户。拆解法院四级审级与五大诉讼阶段、VIAC 2024版紧急仲裁员规则、SIAC裁决在越承认的公共秩序风险与中资企业合同争议解决条款设计。',
        'faq': [
            ('在越南打官司和在越南仲裁，应该怎么选？','仲裁优先。VIAC裁决可通过纽约公约在170+国家执行——越南法院判决的跨境执行通道几乎不存在。法院诉讼的优势是：争议金额低、需要紧急临时措施或涉及破产。'),
            ('SIAC和VIAC应该怎么选？','争议金额超50万美元或对方为越南国企→SIAC（新加坡），裁决国际公信力强、新加坡法院支持仲裁。争议金额较低或对方为越南私企→VIAC，费用远低于SIAC。'),
        ],
        'related': ['15-vietnam-investment-law','35-vietnam-tax','10-asean-arbitration'],
    },
    '53_越南房地产法律实务.md': {
        'slug': '53-vietnam-realestate', 'region': '越南', 'tag': '房地产法',
        'date': '2026-07-31', 'desc': '胡志明市核心区公寓突破5,000美元/㎡，中资占工业用地外资交易38%。拆解2024年《土地法》市场化地价改革、外国人30%公寓配额与50年产权限制、工业用地租赁权抵押新规与代持绝对无效的司法红线。',
        'faq': [
            ('中国人能在越南买地建厂吗？','不能直接买地。外资制造企业须向工业区开发商租赁工业用地（租期最长50年）。2024年《土地法》首次允许外资将租赁权抵押给银行。建议优先选择国家级/省级工业园区内的地块。'),
            ('中国人在越南买房有风险吗？','主要风险：产权期限仅50年（越南公民为长期稳定）、外国人配额限制（单个项目不超30%）、代持被绝对禁止且不受法律保护。购买前须请越南持牌律师完成产权核查和开发商资质核查。'),
        ],
        'related': ['15-vietnam-investment-law','29-vietnam-labor','22-cambodia-land'],
    },
    '54_越南知识产权保护.md': {
        'slug': '54-vietnam-ip', 'region': '越南', 'tag': '知识产权',
        'date': '2026-08-01', 'desc': '中资商标申请量居越南外国首位（7,352件），抢注异议案件同比增长52%。拆解2022年《知识产权法》"恶意抢注"新武器、商标异议2个月黄金窗口、专利侵权证据保全令与海关行政查处的三重执法体系。',
        'faq': [
            ('越南商标被抢注了怎么应对？','分两种情况：商标在公告期（公告后2个月内）→火速提交异议（2022年修订新增"恶意"为独立异议理由）。商标已完成注册→提交无效宣告（理由：恶意抢注或连续5年未使用）。2个月异议期是黄金窗口，错过将转入成本更高的无效宣告程序。'),
            ('中资企业在越南如何进行海关备案保护？','商标/专利/版权在NOIP完成注册后向海关总局提交"知识产权海关保护申请"。海关备案有效期1年（可续展）。海关发现涉嫌侵权货物将在10个工作日内通知权利人——须在10个工作日内要求暂停通关（暂停期限20+20个工作日）。'),
        ],
        'related': ['23-cambodia-ip','07-vietnam-data-localization','15-vietnam-investment-law'],
    },
    '55_柬埔寨公司设立与治理结构.md': {
        'slug': '55-cambodia-company', 'region': '柬埔寨', 'tag': '公司法',
        'date': '2026-08-02', 'desc': '2005年《商业企业法》+2020年单人公司修正案。拆解私人有限公司/分公司/代表处四种进入路径、OBR在线注册、代持致命风险与信托间接持有土地的合规架构。',
        'faq': [
            ('柬埔寨有哪些公司类型？外资应该选哪种？','私人有限公司(Private Limited Company)是最常用形式，2020年修正案允许单人公司。分公司(Branch Office)可开展商业活动但母公司承担无限责任。代表处(Representative Office)仅限市场调研和联络，不得从事经营活动。'),
            ('柬埔寨公司注册资本最低要求是多少？','私人有限公司无法定最低注册资本要求，但建议实缴至少1,000-10,000美元以维持商业信誉。QIP企业通常要求投资额不低于50万美元。'),
            ('通过柬埔寨人代持公司股权合法吗？','不合法。代持安排违反柬埔寨法律，2019年后柬埔寨政府严打代持——代持人可能私下出售股份、死亡后继承人拒绝返还、政府查封涉案资产。信托机制是合规的替代方案。'),
        ],
        'related': ['02-cambodia-qip','22-cambodia-land','18-cambodia-sez'],
    },
    '56_柬埔寨投资争端解决与仲裁.md': {
        'slug': '56-cambodia-arbitration', 'region': '柬埔寨', 'tag': '争议解决',
        'date': '2026-08-03', 'desc': 'NCAC国家商事仲裁中心+SIAC/HKIAC/ICC国际仲裁。拆解仲裁协议七大黄金法则、纽约公约在柬执行的三重障碍、中柬BIT投资条约仲裁(ISDS)与SIAC紧急仲裁员48小时临时措施。',
        'faq': [
            ('在柬埔寨打官司和在柬埔寨仲裁，应该怎么选？','仲裁优先。NCAC仲裁一裁终局、保密、可通过纽约公约跨境执行。法院诉讼的优势场景是需紧急临时措施或涉及破产。建议在合同中约定SIAC新加坡仲裁。'),
            ('外国仲裁裁决在柬埔寨法院执行容易吗？','柬埔寨1960年加入纽约公约，但实务中执行仍是主要挑战——柬埔寨法院对"公共政策例外"的解释较宽泛。建议选择SIAC并约定仲裁地新加坡，可提高裁决质量和执行成功率。'),
            ('中柬之间有没有投资保护协定？','有。中国-柬埔寨BIT(1996年)和中国-东盟投资协定(2009年)均提供投资保护——包括公平公正待遇、征收补偿、资金自由转移等条款。柬埔寨2005年加入ICSID公约，投资者可援引投资条约提起国际仲裁。'),
        ],
        'related': ['10-asean-arbitration','27-cambodia-insolvency','18-cambodia-sez'],
    },
    '57_柬埔寨能源与电力投资法律实务.md': {
        'slug': '57-cambodia-energy', 'region': '柬埔寨', 'tag': '能源法',
        'date': '2026-08-04', 'desc': '2024年柬埔寨宣布不再新建煤电，清洁能源窗口开启。拆解MME/EAC发电许可双轨审批、PPA核心条款（Take-or-Pay/汇率调整/终止补偿）、光伏FIT向竞标转型与中资五大风险管控。',
        'faq': [
            ('柬埔寨电力项目需要哪些许可证？','三类发电许可证：1MW以下免证、1-50MW由EAC审批、50MW以上由MME审批。此外还需要环境部EIA批准、土地租赁权取得和QIP投资优惠申请。整体开发周期约24-42个月。'),
            ('柬埔寨PPA购电协议的谈判重点是什么？','五大核心条款：购电期限(20-30年)、电价公式(容量电价+电量电价)、Take-or-Pay最低购电量、汇率调整机制(收入瑞尔vs贷款美元→汇率错配)、终止补偿与不可抗力。EDC是唯一购电方。'),
            ('柬埔寨可再生能源投资有什么特殊政策？','光伏已从固定FIT转向竞争性招标(auction)机制。水电可采用BOT/BOO模式。风电法律框架仍在制定中。储能(BESS)项目目前缺乏明确的法律定位。2024年柬埔寨宣布不再新建煤电——清洁能源是未来方向。'),
        ],
        'related': ['02-cambodia-qip','25-cambodia-environment','39-fidic-contracts'],
    },
    '58_柬埔寨农业投资法律指引.md': {
        'slug': '58-cambodia-agriculture', 'region': '柬埔寨', 'tag': '农业投资',
        'date': '2026-08-05', 'desc': '中柬FTA零关税+柬埔寨农业资源禀赋。拆解ELC/长期租赁/订单农业四种土地取得路径、外资49%农业公司持股限制、SPS检疫合规、CamGAP认证与七维风险矩阵。',
        'faq': [
            ('外资能在柬埔寨买农业用地吗？','不能。宪法第44条禁止外国人持有农业土地所有权。合法路径包括：经济土地特许(ELC，最长50年，上限10,000公顷)、私人土地长期租赁(50+25年)、订单农业(Contract Farming，最低风险——不与土地权利挂钩)。'),
            ('中国从柬埔寨进口农产品有什么关税优惠？','中柬FTA(2022年生效)对柬埔寨大米、香蕉、芒果、龙眼、胡椒等农产品实施零关税。但每类产品须先签署双边SPS检疫准入议定书。出口时须取得Form E原产地证和CamGAP认证。'),
            ('柬埔寨经济土地特许(ELC)还能申请吗？','2012年Order 01暂停了新ELC的审批。存量ELC续期仍在进行，但新批极少。建议采用私人土地长期租赁或订单农业模式作为替代方案。'),
        ],
        'related': ['22-cambodia-land','02-cambodia-qip','25-cambodia-environment'],
    },
    '59_柬埔寨旅游与酒店业外商投资.md': {
        'slug': '59-cambodia-tourism', 'region': '柬埔寨', 'tag': '行业准入',
        'date': '2026-08-06', 'desc': '2019年《赌场法》+2020年《旅游法》框架。拆解酒店五证齐全许可体系、赌场GMC牌照制度与2019年"818禁赌令"冲击、西港约1,100栋烂尾楼的法律警示与QIP第2类旅游投资优惠。',
        'faq': [
            ('柬埔寨开酒店需要哪些许可证？','五证齐全：商业部公司注册、旅游部酒店经营许可证、建设与土地使用合规、消防证书、环境许可（视规模）。大型酒店可申请QIP第2类优惠——免税期3-6年。'),
            ('柬埔寨赌场牌照现在还能申请吗？','2019年《赌场法》后新牌照审批极为严格。赌场仅限特定经济区/旅游区，金边和暹粒市中心禁止新设赌场。柬埔寨公民禁止进入赌场。2019年"818禁赌令"全面禁止线上博彩——西港约40万人离境，经济遭受重创。'),
            ('中国人在柬埔寨投资酒店用什么方式持有土地？','酒店土地不能外国人直接持有。合法路径：长期租赁(50+25年)、共管公寓一层以上单元(Freehold)、或通过柬埔寨子公司(外资≤49%)间接持有。西港约1,100栋烂尾楼的核心原因之一是投资者采用了不可靠的代持安排。'),
        ],
        'related': ['22-cambodia-land','02-cambodia-qip','06-cambodia-construction'],
    },
    '60_柬埔寨保险业法律监管框架.md': {
        'slug': '60-cambodia-insurance', 'region': '柬埔寨', 'tag': '金融监管',
        'date': '2026-08-07', 'desc': '保险渗透率仅2%的蓝海市场。拆解NBFSA/IRD监管架构、四类保险牌照(RBC偿付能力框架)、外资100%持股准入(东盟最开放)、强制再保险优先分保与中资进入的三阶段策略。',
        'faq': [
            ('外资可以在柬埔寨100%持股保险公司吗？','可以。柬埔寨保险业对外资完全开放，外资可100%持股保险公司——这是东盟最开放的保险外资政策。但须注意：董事会须含柬籍独立董事，且须向Cambodia Re(柬埔寨再保险)优先分保。'),
            ('柬埔寨保险公司的最低资本要求是多少？','非寿险公司400亿瑞尔(≈1,000万美元)、寿险公司800亿瑞尔(≈2,000万美元)、再保险公司1,200亿瑞尔(≈3,000万美元)、小额保险公司60亿瑞尔(≈150万美元)。NBFSA正从固定最低资本向RBC(风险基础资本)框架转型。'),
            ('在柬埔寨的外资企业必须在当地买保险吗？','工程一切险(CAR)、雇主责任险、机动车第三者责任险等属于强制或半强制险种，须向柬埔寨本地持牌保险公司购买。境外保险公司保单在柬埔寨法律效力存疑。建议采用"本地保单+境外再保"的合规结构。'),
        ],
        'related': ['32-cambodia-banking','24-cambodia-fx','21-cambodia-tax'],
    },
    '61_柬埔寨反腐败与商业贿赂合规.md': {
        'slug': '61-cambodia-anticorruption', 'region': '柬埔寨', 'tag': '刑事合规',
        'date': '2026-08-08', 'desc': 'ACU执法高压+中国《刑法》第164条域外管辖+世行ADB交叉制裁=三重风险。拆解反腐败法五大禁止行为、七要素合规体系、第三方KYT尽调与ACU突击搜查的六步危机应对程序。',
        'faq': [
            ('柬埔寨反腐败法处罚有多严厉？','2010年《反腐败法》对贿赂犯罪处5-15年监禁+罚款贿赂金额2-3倍。"公司须为雇员腐败行为承担责任，除非证明已建立充分合规程序"——法人刑事责任倒置。ACU可直接向总理报告，拥有搜查/监听/冻结资产权力。'),
            ('中国企业在柬埔寨行贿还会被中国追诉吗？','会。中国《刑法》第164条规定——中国公民在境外向外国公职人员行贿，中国司法机关具有域外管辖权。加上柬埔寨《反腐败法》和世行/ADB项目触发交叉制裁——中资企业面临三重执法风险。'),
            ('柬埔寨政府官员送礼有什么法律边界？','合理商务宴请可以（需有商业目的+金额合理+记录在案）。但现金/购物卡绝对禁止——无论金额大小。政府官员亲属在公司挂名领薪是ACU重点关注的"高风险指标"。"通融费"(Facilitation Payment)在柬埔寨法律下不豁免。'),
        ],
        'related': ['03-thailand-criminal-defense','13-malaysia-amla','27-cambodia-insolvency'],
    },
    '62_柬埔寨移民法与外籍工作许可.md': {
        'slug': '62-cambodia-immigration', 'region': '柬埔寨', 'tag': '移民法',
        'date': '2026-08-09', 'desc': '签证+工作许可+社保三重合规。拆解C类商务签证EB延期、每年1-3月工作许可申请窗口、外籍禁止从事职业清单(10项)、10%外籍员工比例上限与违规处罚——个人驱逐+黑名单+企业罚款+QIP撤销。',
        'faq': [
            ('中国人在柬埔寨工作需要什么签证和许可？','必须持有C类商务签证(EB延期)——旅游签证(T签)不可在柬埔寨工作，违者可能被驱逐出境。工作许可须在每年1-3月窗口期申请/续期，须提供有效商务签证+劳动合同+无犯罪记录Apostille认证+体检报告。'),
            ('柬埔寨哪些职业禁止外国人从事？','Prakas 360/19禁止外国人从事的职业包括：会计/出纳/收银/前台/司机/导游/律师/公证人等。中资企业须确保外籍员工不从事上述岗位，否则可能被劳动监察部门处罚。'),
            ('违反柬埔寨工作许可规定有什么后果？','个人：驱逐出境+列入黑名单(3-5年不得入境)。企业：行政罚款+暂停招聘外籍员工+影响QIP资格续期。签证逾期每日10美元罚款。万重山律所建议企业建立外籍员工合规到期预警系统。'),
        ],
        'related': ['12-cambodia-labor-law','31-thailand-work-permit','29-vietnam-labor'],
    },
    '63_柬埔寨进出口贸易管制与海关法.md': {
        'slug': '63-cambodia-trade', 'region': '柬埔寨', 'tag': '贸易法',
        'date': '2026-08-10', 'desc': 'ACFTA Form E零关税+RCEP区域累积原产地规则。拆解海关估价与转让定价交叉审查、AEO经认证经营者绿色通道、保税仓储制度与海关稽查三级处罚(低报货值罚50%-200%漏税额)。',
        'faq': [
            ('中国产品出口柬埔寨能享受零关税吗？','大部分可以。ACFTA(中国-东盟自贸区)下大部分产品关税为0-5%。须提供Form E原产地证。RCEP在此基础上新增"区域累积规则"——可在东盟全部成员国+中日韩澳新之间累积原产地成分，对供应链优化是重大利好。'),
            ('柬埔寨进口关税被海关高估了怎么办？','海关须按WTO估价协定的六种估价方法依次适用——交易价格法优先。如海关拒绝接受申报价格，可依次向海关关长申诉→财经部行政复议→法院诉讼。建议在进口前准备充分的交易价格证明材料(合同、发票、银行付款记录等)。'),
            ('柬埔寨海关稽查可以追溯几年？','一般稽查范围为3年，但涉及欺诈/逃税的无时间限制。低报货值处罚为漏税额的50%-200%并可能追究刑事责任。AEO(经认证经营者)可享受绿色通道+低查验率+优先清关——建议有3年合规记录的企业尽早申请。'),
        ],
        'related': ['21-cambodia-tax','24-cambodia-fx','18-cambodia-sez'],
    },
    '64_柬埔寨PPP与基础设施投资.md': {
        'slug': '64-cambodia-ppp', 'region': '柬埔寨', 'tag': '基础设施',
        'date': '2026-08-11', 'desc': '2023年《PPP法》全生命周期管理+三头审批体制(PPP委员会/行业部委/CDC)。拆解BOT/BOO/BTO/DBFOM合同类型、政府担保(PPP法第35条)的法律效力、VGF可行性缺口补贴、汇率错配应对与SIAC仲裁条款设计。',
        'faq': [
            ('柬埔寨PPP项目有哪些合同模式？','2023年《PPP法》规定了BOT(Build-Operate-Transfer)、BOO(Build-Own-Operate)、BTO(Build-Transfer-Operate)、DBFOM(Design-Build-Finance-Operate-Maintain)四种主要合同类型。项目总投资额须≥2,000万美元方可适用PPP法。'),
            ('柬埔寨政府对PPP项目提供担保吗？','PPP法第35条授权政府可对特定PPP项目提供"政府支付担保"——但不构成"主权担保"。实际效力取决于政府财政能力和政治意愿。建议在PPP合同中强化最低收入保证(MRG)、法律变更补偿和终止赔偿条款。'),
            ('中国企业在柬埔寨竞标PPP项目有什么优势？','先发优势明显——通过前期预可研(Pre-feasibility Study)帮助政府完成项目识别和可行性研究，可影响招标技术参数。建议组建"中资施工+中资金融+本地伙伴+国际运营商"的联合体模式，整合技术、资金、本地关系和运营经验四大要素。'),
        ],
        'related': ['06-cambodia-construction','39-fidic-contracts','18-cambodia-sez'],
    },
    '65_泰国2026年BOI新政.md': {
        'slug': '65-thailand-boi-2026', 'region': '泰国', 'tag': '投资法',
        'date': '2026-08-12', 'desc': '2026年BOI新政：A1+级最高13年CIT免税、数据中心列为A1级、第三区叠加优惠倍增。TikTok/AWS/Google已抢先落地，深度拆解EV3.5、审批流程与三国优惠政策横向对比。',
        'faq': [
            ('2026年泰国BOI新政最大的变化是什么？','三大变化：新增A1+级最高13年CIT免税（原上限8年）、超大规模数据中心单独列为A1级、第三区（低收入府）优惠叠加——A1类CIT免税从8年延长至11年+额外3年50%减免。'),
            ('哪些行业在2026年BOI新政中受益最大？','EV电动汽车及零部件（EV 3.5方案）、数据中心与云计算（300亿泰铢以上项目享8+3年CIT减免）、可再生能源与储能、生物技术与精准医疗四大领域。'),
            ('BOI申请需要多长时间？','A类项目45个工作日（2026年缩短后）、B类30个工作日。重大项目（100亿泰铢以上）由BOI委员会月度会议审议。快速通道（Fast Track）可在15个工作日内出具预审意见。'),
            ('不申请BOI有哪些替代方案？','三条替代路径：IEAT工业区许可（审批2-3个月，优惠力度小于BOI）、FBC外资经营许可证（获批率仅约15%）、或调整业务模式使其不受FBA清单限制。'),
        ],
        'related': ['01-thailand-tax-dispute','17-thailand-ma','09-thailand-factory-act'],
    },
    '66_泰国严打代持风暴.md': {
        'slug': '66-thailand-nominee-crackdown', 'region': '泰国', 'tag': '公司合规',
        'date': '2026-08-13', 'desc': 'DSI 2025年查处289起代持案件增长64%。FBA第36条刑事追诉+DBD 2026年泰籍股东亲自到场核实新规，代持模式已走到尽头。详解四条合法100%控股路径与真实重组案例。',
        'faq': [
            ('泰国代持被查处的法律后果是什么？','《外商经营法》第36条：泰籍代持人处3年以下监禁+10万-100万泰铢罚金+持续违规每日1-5万泰铢。法人协助代持罚金翻倍。法院可责令停止经营、解散公司。代持协议在法律上当然无效。'),
            ('2026年DBD代持核查新规有什么变化？','三大变化：泰籍股东须亲自到DBD窗口核实身份+出资来源申报+电子系统自动标红（注册资本超500万但股东年收入不足50万等异常指标）。2026年1月首月即标红1,247家企业。'),
            ('中资企业在泰国如何合法100%控股？','四条合法路径：BOI投资促进（首选，制造业/数据中心/新能源等）、IEAT工业区许可（2-3个月审批）、FBC外资经营许可证（获批率低）、或调整业务至FBA清单外。'),
            ('已被DSI调查的代持企业如何应对？','黄金48小时：立即停止代持安排→委托律师+会计师全面审计→主动提交合规重组计划。DSI目前对主动配合的企业倾向于从轻处罚（行政和解为主），但2026年下半年可能转为全面严打。'),
        ],
        'related': ['17-thailand-ma','01-thailand-tax-dispute','37-thailand-condominium'],
    },
    '67_泰国PDPA天价罚单.md': {
        'slug': '67-thailand-pdpa-enforcement', 'region': '泰国', 'tag': '数据合规',
        'date': '2026-08-14', 'desc': '2025年PDPC罚款1.28亿泰铢增长3倍，单笔最高700万。深度拆解医院数据违规、电商泄露、APP过度收集三大典型案例，五类中资企业高风险场景与PDPA合规行动清单。',
        'faq': [
            ('泰国PDPA 2025年执法有多严格？','2025年PDPC行政处罚47件、罚款1.28亿泰铢（同比增长3倍）、单笔最高700万泰铢。2026年Q1罚款已超6,500万泰铢，全年预计突破3亿泰铢。PDPA"沉睡期"已彻底结束。'),
            ('中资企业最常见的PDPA违规是什么？','五大高频违规：员工数据跨境回传中国（中国未在白名单）、APP过度收集用户数据（默认勾选非必要权限）、CCTV监控违规（休息区/卫生间边界不清）、未经同意直接营销、第三方SDK数据泄露。'),
            ('数据泄露后72小时内必须做什么？','0-4h确认泄露事实和范围→4-24h技术取证和数据映射→24-48h起草PDPC初步报告→48-72h正式提交Form Leak-1。逾期报告本身就是违规。大规模处理数据的机构须提前指定DPO。'),
            ('PDPA、GDPR和PIPL的最大区别是什么？','罚款上限：PDPA 500万泰铢（GDPR为全球营收4%、PIPL为5000万人民币）。跨境传输：PDPA采用SCC+白名单+同意三重机制（GDPR类似、PIPL更严格）。PDPA特色：刑事监禁条款（最高1年）。'),
        ],
        'related': ['11-thailand-pdpa','04-malaysia-pdpa','14-romania-gdpr'],
    },
    '68_柬埔寨2026投资新风口.md': {
        'slug': '68-cambodia-2026-investment', 'region': '柬埔寨', 'tag': '投资法',
        'date': '2026-08-15', 'desc': '2025年第62号Sub-Decree重塑QIP激励体系：光伏制造9年免税、汽车零部件提升至6年、农产品加工首获本地化叠加激励。三大风口行业的投资实践与三国成本对比。',
        'faq': [
            ('2025年第62号Sub-Decree对QIP的主要变化是什么？','三大变革：光伏制造列入第1类QIP享9年免税期、汽车零部件从第3类升至第2类免税6年、农产品加工"本地化叠加"机制（本地采购超70%延长2年+柬籍技术人员超60%再延1年）。双轨制审批：标准20天/快速10天。'),
            ('柬埔寨制造业成本相比越南和印尼有何优势？','月最低工资约220美元（越南380、印尼340），标准厂房租金0.3-0.6美元/㎡/月。核心优势：劳动力成本最低+美元化体系无汇率风险+利润汇回完全自由。短板：工业电价约为越南2倍。'),
            ('中资企业申请柬埔寨QIP需要注意什么？','ODI与QIP双轨协同——先完成中国ODI备案再正式提交QIP申请。合规维护：季度通过e-QIP 2.0提交投资进度报告，连续两季度评分低于60分触发警告。QIP撤销须补缴全部已享税收优惠。'),
        ],
        'related': ['02-cambodia-qip','18-cambodia-sez','21-cambodia-tax'],
    },
    '69_柬埔寨电诈清零后合规投资.md': {
        'slug': '69-cambodia-post-scam', 'region': '柬埔寨', 'tag': '刑事合规',
        'date': '2026-08-16', 'desc': '2024年"清零"行动拘留超5万人。从同一场所"连坐"到好心换汇被牵连——合规企业如何自保？三道法律防线+五大赛道机遇+真实误伤案例全拆解。',
        'faq': [
            ('柬埔寨电诈"清零"行动中合规企业会被误伤吗？','可能。主要风险：与涉诈场所在同一建筑导致整栋查封+银行账户连带冻结、为他人换汇被认定为洗钱通道、合作伙伴暴雷导致关联调查。建议建立三道防线：实体合规+合作伙伴尽调+证据链留存。'),
            ('柬埔寨"清零"行动后西港有哪些投资机会？','五大赛道：中高端酒店与公寓（资产价格较高峰跌60-80%）、冷链物流与供应链（冷库容量仅满足需求20%）、餐饮与生活服务、建材与家装（白名单续建加速）、人力资源服务。'),
            ('在柬埔寨经营如何避免被误认为涉诈？','三条红线绝不可碰：不为涉诈场所提供租赁、不参与人员招聘输送、不提供支付/换汇通道。所有跨境资金通过银行体系、保留完整经营证据链、加入在柬中资商协会。'),
        ],
        'related': ['03-thailand-criminal-defense','61-cambodia-anticorruption','13-malaysia-amla'],
    },
    '70_柔佛新加坡经济特区.md': {
        'slug': '70-johor-singapore-sez', 'region': '马来西亚', 'tag': '投资法',
        'date': '2026-08-17', 'desc': 'JS-SEZ覆盖3,505km²，已签约750亿林吉特。5%企业所得税特惠税率+15%知识工作者个税。深度拆解数据中心/半导体/医疗旅游/新能源四大产业机遇与法律实操要点。',
        'faq': [
            ('柔佛-新加坡经济特区（JS-SEZ）最大的政策亮点是什么？','5%企业所得税特惠税率（标准24%，降幅79%），有效期最长15年。条件：实质性经营+雇用≥50名合格员工+年运营支出≥1,000万林吉特+拥有/租赁办公场所。知识工作者享15%固定个税。'),
            ('JS-SEZ与深圳-香港模式有何异同？','同：依托金融中心（新加坡/香港）+低成本制造基地（柔佛/深圳）+税收优惠。异：JS-SEZ是两国间跨境合作（涉及两个海关关境），深圳模式是一国内部合作。JS-SEZ面积（3,505km²）约为深圳的1.7倍。'),
            ('中资企业在JS-SEZ投资需要注意什么法律问题？','四大要点：资本弱化限制（关联借贷/股本≤3:1）、受益所有人穿透申报（至最终自然人）、土地收购须州政府批准（JS-SEZ快速通道3个月）、外籍高管工作准证审批缩短至5个工作日。'),
        ],
        'related': ['42-malaysia-foreign-investment','41-malaysia-tax','18-cambodia-sez'],
    },
    '71_马来西亚数据中心投资潮.md': {
        'slug': '71-malaysia-data-center', 'region': '马来西亚', 'tag': '数字经济',
        'date': '2026-08-18', 'desc': '柔佛在建超20个超大规模数据中心，AWS/Google/字节跳动/万国数据已落地。拆解新加坡溢出效应、能源审批、PDPA合规与中资企业三种参与模式的实操路径。',
        'faq': [
            ('为什么马来西亚能成为东南亚数据中心枢纽？','三大驱动力：新加坡暂停新数据中心审批的溢出效应（柔佛距新加坡仅30-60分钟车程）、电价仅为新加坡的45%、政府战略驱动——Pioneer Status使数据中心实际税率从24%降至7.2%。'),
            ('中资企业参与马来西亚数据中心投资有哪些模式？','三种模式：自建运营（头部云服务商，投资30-50亿林吉特级）、与当地企业合资（降低进入门槛+利用本地审批通道）、配套服务商（设备供应/EPC工程——投资门槛最低）。'),
            ('马来西亚数据中心投资面临哪些合规要求？','MIDA投资审批（通常≥1亿林吉特）、能源委员会电力供应许可、EIA环评（≥50MW）、PDPA数据保护合规（数据处理者直接责任+跨境传输限制）、州政府土地审批（JS-SEZ内约3个月）。'),
        ],
        'related': ['04-malaysia-pdpa','42-malaysia-foreign-investment','26-cambodia-digital'],
    },
    '72_马来西亚新并购管制.md': {
        'slug': '72-malaysia-merger-control', 'region': '马来西亚', 'tag': '竞争法',
        'date': '2026-08-19', 'desc': '2025年MyCC正式启动并购申报制度。全球20亿+本地4,000万令吉触发门槛，30+90天两阶段审查。首年32件申报、首例附条件批准（资产剥离）——中资企业并购前六步自查清单。',
        'faq': [
            ('马来西亚并购管制的申报门槛是多少？','全球营业额合计超20亿令吉+马来西亚境内至少两家各超4,000万令吉，或兼并后市场份额≥20%。注意：计算口径为集团合并报表，非仅交易标的。马来西亚门槛约为中国SAMR的1/5。'),
            ('MyCC审查需要多长时间？交易可以提前交割吗？','不可以——抢跑（gun-jumping）面临全球营业额10%罚款+法院拆分令。第一阶段30个工作日、第二阶段90+30个工作日。建议将MyCC审查时间纳入交易时间表，SPA中列入"获批为交割先决条件"。'),
            ('中资企业收购马来西亚公司30%股权需要申报吗？','可能需要。MyCC对"控制权"的定义包括：持股30%以上且伴有特别表决权/董事任命权/战略决策否决权。即使持股不超50%，只要具备"事实控制"即触发申报义务。'),
        ],
        'related': ['19-malaysia-competition','17-thailand-ma','13-malaysia-amla'],
    },
    '73_罗马尼亚欧盟资金窗口.md': {
        'slug': '73-romania-eu-funds', 'region': '罗马尼亚', 'tag': '投资法',
        'date': '2026-08-20', 'desc': 'PNRR 291亿欧元+凝聚基金460亿欧元，2026年8月NGEU截止。公路/铁路/能源/水利四大招标密集发布。拆解IPI/FSR/本地化三道关卡与中资企业联合体投标策略。',
        'faq': [
            ('罗马尼亚欧盟资金2026年的关键时间节点是什么？','PNRR第六批付款申请截止2026年6月30日、NGEU资金最终付款截止2026年8月31日、2021-2027凝聚基金n+3规则关键节点2026年12月31日。2026年是欧盟资金使用的"决定性年份"。'),
            ('中资企业参与罗马尼亚公共采购面临哪些法律障碍？','三道关卡：IPI条例（限制非对等市场准入国企业）、FSR外国补贴条例（2.5亿欧元以上合同须申报过去三年外国补贴）、技术标准与本地业绩要求（EN标准认证+5年欧盟内类似业绩）。'),
            ('中资企业如何有效进入罗马尼亚基建市场？','首推联合体模式：中资提供技术和资金+罗马尼亚本地企业提供施工资质和业绩。合同须纳入FIDIC调价机制（罗马尼亚近年通胀高达16.8%），争议解决建议ICC仲裁而非罗国内法院。'),
        ],
        'related': ['34-romania-company-law','44-romania-tax','45-romania-renewable-energy'],
    },
    '74_乌克兰重建罗马尼亚枢纽.md': {
        'slug': '74-ukraine-reconstruction-romania', 'region': '罗马尼亚', 'tag': '地缘投资',
        'date': '2026-08-21', 'desc': '乌克兰重建需求4,860亿美元。康斯坦察港成为重建物资核心枢纽，罗乌边境货运量增长42%。三种实操模式：设厂出口+联合体竞标+贸易中转，东方雨虹等中企已先行落地。',
        'faq': [
            ('中资企业如何通过罗马尼亚参与乌克兰重建？','三种模式：在罗设厂→加工为欧盟原产→DCFTA零关税出口乌克兰（建材企业首选）、与罗承包商组建联合体→竞标世行/EIB/欧盟资助项目（工程企业）、在罗注册贸易公司+保税仓→物流中转（贸易/物流企业）。'),
            ('通过罗马尼亚向乌克兰转运物资有哪些合规风险？','三大硬伤：欧盟制裁合规（须核查最终用户是否受制裁+商品是否军民两用+运输路线是否途经俄/白俄）罗马尼亚海关2025年Q1已查扣350万欧元涉嫌违规电子产品。建议运输前委托罗律师做制裁尽调。'),
            ('在乌克兰境内投资如何保护资产安全？','三管齐下：购买政治风险保险（MIGA或中国信保战争险）、合同中约定ICSID/UNCITRAL国际仲裁排除乌国内法院管辖、不可抗力条款至少覆盖战争状态变化+边境关闭替代路线+NBU付款延迟安排。'),
        ],
        'related': ['34-romania-company-law','73-romania-eu-funds','47-romania-fdi'],
    },
    '75_泰国数字银行与金融科技监管.md': {
        'slug': '75-thailand-digital-banking', 'region': '泰国', 'tag': '金融监管',
        'date': '2026-08-22', 'desc': '2026年泰国首批3张虚拟银行牌照发放，蚂蚁集团和微众银行入局。深度拆解虚拟银行FBA 49%外资限制、数字资产SEC监管、开放银行三层架构与金融科技六大合规要点。',
        'faq': [
            ('泰国虚拟银行牌照外资可以100%持股吗？','不可以。金融服务业适用《外商经营法》(FBA)49%外资上限。中资企业参与路径：技术合作伙伴模式（不持股权）或少数股权+技术合作模式（≤49%）。蚂蚁集团（与正大集团合资）和微众银行（与SCBX合资）即采用少数股权+技术合作模式。'),
            ('泰国数字资产交易所需要什么牌照？','须取得SEC颁发的数字资产交易所牌照，最低注册资本5,000万泰铢，外资持股上限49%。泰国SEC采用"效果原则"——即使服务器不在泰国，面向泰国居民提供服务的境外平台也须在SEC注册。'),
            ('中资金融科技企业进入泰国市场的合规要点是什么？','六大要点：FBA外资限制分析、PDPA数据跨境传输限制（中国不在白名单）、BOT数据本地化要求、最低注册资本和牌照申请、AMLO反洗钱合规体系、与监管部门的事前沟通（Pre-Application Consultation）。'),
        ],
        'related': ['17-thailand-ma','11-thailand-pdpa','01-thailand-tax-dispute'],
    },
    '76_柬埔寨数据保护法前瞻.md': {
        'slug': '76-cambodia-data-protection', 'region': '柬埔寨', 'tag': '数据合规',
        'date': '2026-08-23', 'desc': '柬埔寨《个人数据保护法》草案预计2027年颁布。十条核心制度深度解读：域外管辖、七项合法性基础、敏感数据（含财务数据）特殊保护、DPO强制化、72小时泄露通知、跨境SCC。',
        'faq': [
            ('柬埔寨数据保护法什么时候生效？','柬埔寨《个人数据保护法》(PDPL)第三版草案已于2026年6月发布，预计2026年底至2027年初提交国会审议，2027年内正式颁布。柬埔寨律师余驰宇建议中资企业现在（2026年）就启动数据合规准备。'),
            ('柬埔寨PDPL和GDPR有什么主要区别？','PDPL草案在核心制度上借鉴了GDPR（域外管辖/DPO/DPIA/72小时泄露通知/SCC），但有两处超越GDPR的创新：财务数据和地理位置数据被列为"敏感数据"、引入了数据泄露的"集体诉讼"机制。'),
            ('中资企业在柬埔寨的APP需要做什么合规准备？','须在PDPL生效前完成：隐私政策和用户同意机制的升级（默认勾选无效）、数据映射(Data Mapping)、跨境数据传输SCC签署、数据泄露应急响应预案、DPO的指定和向CDPC报备。'),
        ],
        'related': ['26-cambodia-digital','04-malaysia-pdpa','11-thailand-pdpa'],
    },
    '77_马来西亚可再生能源与氢能经济.md': {
        'slug': '77-malaysia-renewable-energy', 'region': '马来西亚', 'tag': '能源法',
        'date': '2026-08-24', 'desc': 'NETR第二阶段2,500亿林吉特清洁能源投资计划。LSS 6预计招标2,500MW+，氢能经济路线图启动，Bursa Carbon Exchange碳信用交易增长156%。全面拆解太阳能、储能、绿电出口三大赛道的法律框架。',
        'faq': [
            ('马来西亚LSS 6太阳能招标何时启动？','预计2026年下半年启动，招标容量2,500MW+。可能的创新包括太阳能+储能一体化项目（4小时储能配套）和外资持股上限从49%提升到70%（针对储能项目）。外资须与Bumiputera企业组建合资公司。'),
            ('马来西亚可再生能源外资可以100%持股吗？','发电领域外资持股上限为49%，须与马资（特别是Bumiputera资本）合资。但储能一体化项目和绿氢生产的外资上限可能放宽至70%。EPC总承包和O&M运维可以由外资100%持股的企业承担。'),
            ('马来西亚太阳能项目的PPA期限有多长？','LSS竞标太阳能PPA期限为21年（含通货膨胀指数调整），TNB为唯一购电方。PPA的终止赔偿覆盖未来剩余年限电费收入的净现值。马来西亚律师余驰宇建议在PPA中明确储能调度规则和容量衰减的损失承担机制。'),
        ],
        'related': ['42-malaysia-foreign-investment','41-malaysia-tax','51-malaysia-construction'],
    },
    '78_罗马尼亚数字服务法与AI法案.md': {
        'slug': '78-romania-dsa-ai-act', 'region': '罗马尼亚', 'tag': '数字经济',
        'date': '2026-08-25', 'desc': 'DSA 2026年全面生效+AI Act高风险合规截止。四层递进义务体系、四类AI风险分级、五种中资高频触发场景和ANCOM/ANSPDCP双重监管下的合规行动清单。',
        'faq': [
            ('罗马尼亚DSA对中资电商平台有什么要求？','在罗运营的电商平台须：建立"通知-行动"机制（7个工作日内处理用户举报）、入驻卖家KYC（收集并验证身份/地址/税务号）、在罗境内指定法律代表或联络点（须能用罗语进行监管沟通）、发布罗语版用户条款和内容管理政策。'),
            ('欧盟AI Act 2026年的关键时间节点是什么？','2026年5月2日——高风险AI系统合规要求全面生效。在罗中资企业使用AI进行招聘筛选、员工绩效评估、工业安全监控的，须在此日期前完成风险管理体系+技术文档+人工监督机制+合格评定。2026年8月2日——通用AI模型规则生效。'),
            ('违反AI Act的罚款上限是多少？','不可接受风险AI（如实时人脸识别）：最高3,500万欧元或全球年营收7%。高风险AI违规：最高1,500万欧元或全球年营收3%。对中资大型科技企业，7%全球营收的罚款具有实质威慑力。'),
        ],
        'related': ['14-romania-gdpr','34-romania-company-law','47-romania-fdi'],
    },
    '79_越南可再生能源与EV投资.md': {
        'slug': '79-vietnam-renewable-energy-ev', 'region': '越南', 'tag': '能源法',
        'date': '2026-08-26', 'desc': 'PDP8实施计划启动，2030年太阳能+风电新增40GW。DPPA私人线路直供+屋顶太阳能自发自用合法化。EV电池制造享10% CIT优惠15年+4免9减半。五维风险防控和投资架构设计。',
        'faq': [
            ('越南DPPA直接购电协议有几种模式？','两种：模式一（物理DPPA）通过私人线路直供，电价自由协商，不经过EVN电网——最具商业吸引力；模式二（虚拟DPPA）通过EVN电网+金融CfD差价合约。私人线路DPPA须满足同一或相邻工业园区、不超过省级边界、电压≤35kV等限制。'),
            ('越南屋顶太阳能有什么最新政策？','2025年第135号法令：自发自用余电上网模式合法化，余电上网比例不超过20%，上网电价约为EVN平均批发电价的70%。免于办理发电许可证。企业所得税优惠——投资额的15%可抵扣当年应纳税所得。'),
            ('越南EV电池制造享有哪些税收优惠？','2026年6月新法令：CIT 10%优惠税率15年+4免9减半+进口设备免关税免增值税+土地使用权租金前15年全免。实际CIT税率可从标准20%降至约1.2%（前4年）。越南律师余驰宇建议2026年Q4前完成选址和ODI申请，锁定当前政策窗口。'),
        ],
        'related': ['15-vietnam-investment-law','35-vietnam-tax','29-vietnam-labor'],
    },
    '80_印尼清真认证与食品医药监管.md': {
        'slug': '80-indonesia-halal', 'region': '印尼', 'tag': '行业准入',
        'date': '2026-08-27', 'desc': '2026年10月17日Halal认证全面强制化——十大类产品须有Halal证方可流通。BPJPH+MUI+LPH三权分立新制度，五种认证路径，中国Halal认证MRA谈判进展。3,200亿美元市场门槛全解析。',
        'faq': [
            ('2026年10月17日后，在印尼没有Halal认证的产品会被怎么处理？','市场禁售、产品下架、召回及销毁（费用企业承担）、行政罚款上限50亿印尼盾、吊销营业许可。BPJPH将与BPOM和海关总署联动执法——无Halal证的产品在海关直接扣留。管理者/负责人可被处6个月至5年监禁。'),
            ('中国已有的清真认证在印尼可以直接用吗？','不一定。须看中国认证机构是否在BPJPH认可的76家外方Halal认证机构清单中。即使被认可，互认范围可能仅限特定产品类别。最稳妥的做法：委托印尼律师余驰宇向BPJPH进行预咨询(Izin Konsultasi)，确认认证互认。'),
            ('Halal认证需要多长时间和费用？','简单产品（纯净水/初级农产品）：2-4周（自我声明路径）、500-1,500万印尼盾。中等复杂度（加工食品/调味料）：3-6个月、2,000-5,000万印尼盾。高复杂度（药品/化妆品）：6-12个月、5,000-15,000万印尼盾。2026年10月截止前启动认证的最后窗口期。'),
        ],
        'related': ['16-indonesia-omnibus','08-indonesia-ikn','36-indonesia-pdp'],
    },
    '81_柬埔寨储能与碳信用法律框架.md': {
        'slug': '81-cambodia-energy-storage-carbon', 'region': '柬埔寨', 'tag': '能源法',
        'date': '2026-08-28', 'desc': '柬埔寨宣布不再新建煤电，太阳能+储能成为投资新赛道。BESS法律定位初探、太阳能+储能PPA谈判五大要点、碳信用1.2亿tCO2e/年潜力和国家碳登记系统的合规要求。',
        'faq': [
            ('柬埔寨储能(BESS)项目需要什么许可？','柬埔寨尚未出台专门的储能法。共址储能（与太阳能共享场址）适用发电许可——由MME和EAC审批，这是目前最成熟的路径。独立储能法律定位不明确。柬埔寨律师余驰宇建议从太阳能+储能共址模式起步。'),
            ('柬埔寨的碳信用项目有什么法律要求？','柬埔寨碳信用须通过环境部国家碳登记系统注册。REDD+碳信用归柬埔寨国家所有。可再生能源碳信用由项目投资者按协议份额享有收入。碳信用出口须经环境部审批+缴纳碳排放权出让费（预计交易额2-5%）。'),
            ('太阳能+储能PPA的储能条款有哪些谈判要点？','五大要点：储能容量电价($/MW/月)——独立于发电量电价的增项；储能调度规则——EDC的调度权限范围；循环寿命和容量衰减——PPA保证多少个完整充放电循环；储能替代/升级——电池替换费用由谁承担；最低可接受容量(MAC)和损失分担机制。'),
        ],
        'related': ['57-cambodia-energy','02-cambodia-qip','25-cambodia-environment'],
    },
    '82_马来西亚数字银行与加密货币监管.md': {
        'slug': '82-malaysia-digital-banking-crypto', 'region': '马来西亚', 'tag': '金融监管',
        'date': '2026-08-29', 'desc': '五张数字银行牌照运营一年回顾：890万客户+85亿林吉特存款。SC数字资产指引2026修订：加密ETF、稳定币框架、DeFi监管、DAO法律地位。中资金融科技两条进入路径全拆解。',
        'faq': [
            ('马来西亚数字银行牌照还会发放吗？','目前BNM尚未宣布第二轮牌照计划。中资企业当前路径：成为现有五家数字银行的技术供应商（核心银行系统/移动APP/AI风控/e-KYC/反欺诈系统），或通过后续增资轮次以战略少数股权（≤20%）进入。马来西亚律师余驰宇建议以"技术许可+本地化实施"模式进入。'),
            ('在马来西亚经营加密货币交易所需要什么牌照？','须取得SC认可的"认可市场运营商"(RMO)资格，含DAX（数字资产交易所）类别。截至2026年6月，SC已认可4家DAX：Luno、Sinegy、Tokenize、MX Global。中资加密企业可通过战略投资现有持牌DAX（如币安投资MX Global的先例）或申请新RMO牌照。'),
            ('马来西亚对DAO有什么法律立场？','DAO在马来西亚现行法律下无独立法人地位——不能持有资产、签合同、起诉/被诉。SC 2026年提议承认特定类型的DAO为有限合伙(LLP)，条件是DAO的治理规则符合LLP法要求。目前仍处于咨询阶段。'),
        ],
        'related': ['04-malaysia-pdpa','13-malaysia-amla','19-malaysia-competition'],
    },
    '83_欧盟CBAM碳边境调节机制与东南亚应对.md': {
        'slug': '83-eu-cbam-asean-carbon', 'region': '跨境', 'tag': '碳合规',
        'date': '2026-08-30', 'desc': 'CBAM 2026年正式征收，EU ETS碳价约88欧元/tCO2。六国碳合规能力横向对比：柬埔寨和罗马尼亚电网最清洁、泰马碳市场最成熟。钢铁/铝/水泥/化肥出口欧盟的成本冲击与三项应对策略。',
        'faq': [
            ('CBAM对中国在东南亚的工厂有什么影响？','东南亚生产的钢铁/铝/水泥/化 fertilizer出口欧盟时须缴纳CBAM证书费用。以越南钢铁厂为例：每吨粗钢嵌入式碳排放约2.0tCO2，CBAM证书成本约176欧元/吨（2.0×88欧元）。降低碳排放的最有效方式是切换至可再生能源电力+安装碳排放连续监测系统(CEMS)。'),
            ('东南亚哪个国家的CBAM应对能力最强？','罗马尼亚（已在EU ETS内，合规路径最清晰）+马来西亚（BCX自愿碳市场成熟+政府推动碳税）。柬埔寨电网最清洁（水电占62%排放因子仅0.35tCO2e/MWh）但碳制度最不成熟。越南和印尼电网排放因子最高（0.60-0.70tCO2e/MWh），需要可再生能源直供。'),
            ('中国碳市场的碳价能抵扣CBAM吗？','可以。CBAM允许扣减在原产国已支付的碳价。中国全国碳市场碳价约75元人民币/tCO2（≈9.5欧元/tCO2），差额约78欧元/tCO2须通过CBAM证书补缴。东南亚企业当前没有碳定价——可抵扣额为零。跨境投资律师余驰宇建议在东南亚提前布局可再生能源和碳信用。'),
        ],
        'related': ['45-romania-renewable-energy','57-cambodia-energy','77-malaysia-renewable-energy'],
    },
    '84_中资出海东盟十维法律尽调手册.md': {
        'slug': '84-asean-due-diligence', 'region': '跨境', 'tag': '投资法',
        'date': '2026-08-31', 'desc': '从三起失败案例出发：外资准入、土地权属、公司治理、税务激励、劳动法、持续合规、ESG、外汇管理、知识产权、争议解决——十维尽调框架+行业权重矩阵+投资可行性红绿灯系统。',
        'faq': [
            ('中资出海东南亚投资前必须做哪些法律尽调？','涉外律师余驰宇总结十维尽调框架：①外资准入与国家安全审查②土地权属③公司治理④税务与激励⑤劳动法⑥持续合规⑦ESG⑧外汇管理⑨知识产权⑩争议解决。不同行业权重不同：制造业土地和劳动法五星、能源ESG五星、数字科技知识产权五星、金融科技外汇和持续合规五星。'),
            ('东南亚投资最常见的一票否决风险是什么？','三大一票否决风险：外资准入绝对禁止——目标行业不允许外资持股；土地代持陷阱——代持在泰/柬/越/印尼均无效且面临刑事追诉；中国ODI备案无法通过——属于敏感行业/敏感国家。涉外律师余驰宇建议在项目早期委托当地律师进行监管预沟通，了解实际审批尺度。'),
            ('尽调发现"黄灯"怎么办？','在一个新市场（尤其柬埔寨等法律体系发展中国家），十维尽调通常有3-5个黄灯。关键是区分"可随时间变绿的黄灯"（申请QIP/BOI激励、与政府建立关系、签订完善的合同）和"根本性黄灯"（可能走向红灯的潜在风险）。对前者制定"缓解方案"并设置"补救条款"。'),
        ],
        'related': ['30-bri-risk-map','20-china-odi','10-asean-arbitration'],
    },
}

SKIP_LINES = {'余驰宇', '上海', '律师', '关注', '举报/反馈', '评论', '发表', '作者最新文章',
              '换一换', '扫码下载百度APP', '搜最新资讯、看热门视频', '设为首页',
              '我是余驰宇，中国执业律师', '请关注我，私信交流。'}

def normalize_markdown(text):
    text = re.sub(r'^##\s+', '', text, flags=re.MULTILINE)
    text = re.sub(r'^###\s+', '', text, flags=re.MULTILINE)
    return text

def extract_toc(text):
    toc = []
    for line in text.split('\n'):
        line = line.strip()
        if re.match(r'^[一二三四五六七八九十]、', line):
            toc.append(('h2', line))
        elif line in ('引言', '结语', '问题的提出'):
            toc.append(('h2', line))
        elif re.match(r'^（[一二三四五六七八九十]）', line):
            toc.append(('h3', line))
    return toc

def generate_toc_html(toc):
    if len(toc) < 3: return ''
    html = '<nav class="article-toc"><h2>目录</h2><ol>\n'
    for level, title in toc:
        cls = 'toc-h2' if level == 'h2' else 'toc-h3'
        html += f'  <li class="{cls}"><a href="#{title}">{title}</a></li>\n'
    html += '</ol></nav>\n'
    return html

def generate_faq_schema(faq):
    if not faq: return ''
    items = ',\n'.join([
        f'    {{"@type":"Question","name":"{q}","acceptedAnswer":{{"@type":"Answer","text":"{a}"}}}}'
        for q, a in faq
    ])
    return f'<script type="application/ld+json">\n{{\n  "@context":"https://schema.org",\n  "@type":"FAQPage",\n  "mainEntity":[\n{items}\n  ]\n}}\n</script>\n'

def generate_related_html(related_slugs, all_files, all_meta, current_file, region, tag, limit=5):
    """Generate related articles HTML with auto-supplement from same region/tag."""
    seen = set(related_slugs)

    # First collect manual related
    items = []
    for slug in related_slugs:
        for fname, meta in all_meta.items():
            if meta.get('slug') == slug:
                path = os.path.join(ARTICLES_DIR, fname)
                if os.path.exists(path):
                    with open(path, 'r', encoding='utf-8') as fh:
                        title = fh.readline().replace('# ', '').strip()
                    items.append((slug, title))
                break

    # Auto-supplement: same region + same tag, excluding current article
    if len(items) < limit:
        auto_candidates = []
        for fname, meta in all_meta.items():
            mslug = meta.get('slug', '')
            if mslug in seen or fname == current_file:
                continue
            score = 0
            if meta.get('region') == region:
                score += 2
            if meta.get('tag') == tag:
                score += 1
            if score > 0:
                auto_candidates.append((score, mslug, fname))
        auto_candidates.sort(key=lambda x: x[0], reverse=True)
        for score, mslug, fname in auto_candidates:
            if len(items) >= limit: break
            if mslug in seen: continue
            seen.add(mslug)
            path = os.path.join(ARTICLES_DIR, fname)
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as fh:
                    title = fh.readline().replace('# ', '').strip()
                items.append((mslug, title))

    if not items: return ''
    html = '<nav class="related-articles"><h2>相关文章</h2><ul>\n'
    for slug, title in items:
        html += f'  <li><a href="/articles/{slug}.html">{title}</a></li>\n'
    html += '</ul></nav>\n'
    return html


def generate_tldr(desc, faq):
    """Generate TL;DR key points from article description and FAQ."""
    points = []
    # Point 1: from description
    if desc:
        points.append(desc.rstrip('。'))

    # Points 2-3: from first two FAQ answers (summarized)
    for i, (q, a) in enumerate(faq[:2]):
        # Truncate long answers to ~80 chars
        short_a = a[:80].rstrip('。') + '……' if len(a) > 80 else a
        points.append(f'<strong>{q}</strong>：{short_a}')

    if not points: return ''
    html = '<div class="article-tldr"><h3>核心要点</h3><ol>\n'
    for p in points:
        html += f'  <li>{p}</li>\n'
    html += '</ol></div>\n'
    return html

def md_to_html(text):
    text = normalize_markdown(text)
    lines = text.split('\n')
    html = []
    in_ol = False; in_ul = False

    def close_lists():
        nonlocal in_ol, in_ul
        if in_ol: html.append('</ol>'); in_ol = False
        if in_ul: html.append('</ul>'); in_ul = False

    for line in lines:
        line = line.strip()
        if not line: close_lists(); continue
        if line in SKIP_LINES: continue
        if re.match(r'^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}', line): continue
        if line.startswith('©') or line.startswith('京ICP'): continue
        if re.match(r'^\d+$', line) and len(line) <= 3: continue
        if '百度首页' in line or '余驰宇律师' in line: continue
        if len(line) < 5 and not re.match(r'^[一二三四五六七八九十]、', line): continue

        is_h2 = False
        if re.match(r'^[一二三四五六七八九十]、', line): is_h2 = True
        elif line in ('引言', '结语', '问题的提出'): is_h2 = True
        if is_h2:
            close_lists()
            html.append(f'<h2 id="{line}">{line}</h2>')
            continue

        if re.match(r'^（[一二三四五六七八九十]）', line):
            close_lists(); html.append(f'<h3>{line}</h3>'); continue

        line = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', line)
        if re.match(r'^\d+[\.\、]', line):
            if not in_ol: close_lists(); html.append('<ol>'); in_ol = True
            cleaned = re.sub(r'^\d+[\.\、]\s*', '', line)
            html.append(f'<li>{cleaned}</li>')
            continue
        if line.startswith('- '):
            if not in_ul: close_lists(); html.append('<ul>'); in_ul = True
            html.append(f'<li>{line[2:]}</li>')
            continue

        close_lists(); html.append(f'<p>{line}</p>')

    close_lists()
    return '\n'.join(html)


HTML_ARTICLE = '''<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="icon" type="image/x-icon" href="/favicon.ico">
<link rel="icon" type="image/png" sizes="32x32" href="/images/favicon-32x32.png">
<link rel="icon" type="image/png" sizes="16x16" href="/images/favicon-16x16.png">
<link rel="apple-touch-icon" sizes="180x180" href="/images/apple-touch-icon.png">
<meta name="baidu-site-verification" content="codeva-clmOQd5j7H" />
<title>{title} | 余驰宇律师</title>
<meta name="description" content="{desc}">
<meta name="keywords" content="{region}法律,{tag},跨境投资,余驰宇律师,上海涉外律师,境外投资,海外合规,中国企业出海,涉外法律服务,东南亚投资">
<meta name="author" content="余驰宇">
<link rel="canonical" href="https://chiryyu.com/articles/{slug}.html">
<meta property="og:title" content="{title}">
<meta property="og:description" content="{desc}">
<meta property="og:image" content="https://chiryyu.com/images/{cover}">
<meta property="og:url" content="https://chiryyu.com/articles/{slug}.html">
<meta property="og:type" content="article">
<meta property="og:site_name" content="余驰宇律师 | 跨境投资法律实务">
<meta property="og:locale" content="zh_CN">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{title}">
<meta name="twitter:description" content="{desc}">
<meta name="twitter:image" content="https://chiryyu.com/images/{cover}">
<link rel="stylesheet" href="/css/style.css">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-RNSF9MHKRC"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){{dataLayer.push(arguments);}}
  gtag('js', new Date());
  gtag('config', 'G-RNSF9MHKRC');
</script>
<script>
var _hmt = _hmt || [];
(function() {{
  var hm = document.createElement("script");
  hm.src = "https://hm.baidu.com/hm.js?4f699fb9163cce274dc69ba8316bb3e8";
  var s = document.getElementsByTagName("script")[0];
  s.parentNode.insertBefore(hm, s);
}})();
</script>
<script type="application/ld+json">
{{
  "@context": "https://schema.org",
  "@type": "Article",
  "headline": "{title}",
  "description": "{desc}",
  "image": "https://chiryyu.com/images/{cover}",
  "author": {{ "@type": "Person", "name": "余驰宇", "alternateName": "Yu Chiyu", "url": "https://chiryyu.com/about.html", "image": "https://chiryyu.com/images/avatar.png", "jobTitle": "中国执业律师", "description": "跨境投资律师，专注柬埔寨、泰国、马来西亚、罗马尼亚等国法律实务", "sameAs": ["https://chiryyu.com/about.html", "https://shanghai.dacheng.com/lawyer_1/43.html"] }},
  "datePublished": "{date}T08:00:00+08:00",
  "dateModified": "{last_updated}T08:00:00+08:00",
  "publisher": {{ "@type": "Person", "name": "余驰宇" }},
  "mainEntityOfPage": {{ "@type": "WebPage", "@id": "https://chiryyu.com/articles/{slug}.html" }},
  "articleSection": "{region} · {tag}"
}}
</script>
<script type="application/ld+json">
{{
  "@context":"https://schema.org",
  "@type":"BreadcrumbList",
  "itemListElement":[
    {{"@type":"ListItem","position":1,"name":"首页","item":"https://chiryyu.com/"}},
    {{"@type":"ListItem","position":2,"name":"文章","item":"https://chiryyu.com/articles/"}},
    {{"@type":"ListItem","position":3,"name":"{title}"}}
  ]
}}
</script>
{faq_schema}
</head>
<body>

<header class="site-header">
  <div class="header-inner">
    <a href="/" class="site-title">余驰宇<span>律师</span></a>
    <nav class="site-nav">
      <a href="/">首页</a>
      <a href="/articles/">文章</a>
      <a href="/cambodia-lawyer.html">柬埔寨律师</a>
      <a href="/about.html">关于</a>
    </nav>
  </div>
</header>

<article>
  <header class="article-header">
    <nav class="breadcrumb"><a href="/">首页</a> › <a href="/articles/">文章</a> › <span>{region} · {tag}</span></nav>
    <img src="/images/{cover}" alt="{title}" class="article-cover" width="800" height="450" loading="lazy">
    <div class="article-region">{region} · {tag}</div>
    <h1>{title}</h1>
    <div class="article-meta">余驰宇 · {date} · 阅读约{readtime}分钟</div>
  </header>
  <div class="article-body">
<div class="article-summary">{desc}</div>
{tldr}
{toc}
{body}
{related}
  </div>
  <div class="author-bio">
    <img src="/images/avatar.png" alt="余驰宇律师" class="author-avatar" width="80" height="80">
    <div class="author-info">
      <strong>余驰宇</strong>，中国执业律师，大成律师事务所，牵头柬埔寨、泰国、马来西亚、罗马尼亚等国当地律所的中国业务部。专注中资企业海外财产保护、重大诉讼/仲裁、税务问题处理、跨境保全与执行。
      <div class="author-contact">📞 <a href="tel:+8615201911206">15201911206</a> · 微信 chiry003</div>
    </div>
  </div>
</article>

<footer class="site-footer">
  <p>© 2026 余驰宇律师 · <a href="/">chiryyu.com</a></p>
  <p style="margin-top:0.5rem;">📞 <a href="tel:+8615201911206">15201911206</a> · 微信 chiry003</p>
  <p style="margin-top:0.3rem;"><a href="/articles/">← 返回文章列表</a></p>
</footer>

</body>
</html>'''


# ===== BUILD =====
all_files = sorted([f for f in os.listdir(ARTICLES_DIR) if f.endswith('.md')])
cambodia_files = [f for f in all_files if '柬埔寨' in f]
other_files = [f for f in all_files if '柬埔寨' not in f]
files = cambodia_files + other_files

for f in files:
    meta = META.get(f, {})
    slug = meta.get('slug', '')
    region = meta.get('region', '跨境')
    tag = meta.get('tag', '法律')
    date = meta.get('date', '')
    desc = meta.get('desc', '')
    faq = meta.get('faq', [])
    related = meta.get('related', [])

    path = os.path.join(ARTICLES_DIR, f)
    with open(path, 'r', encoding='utf-8') as fh:
        raw = fh.read()

    lines = raw.split('\n')
    title = lines[0].replace('# ', '').strip()
    body_md = '\n'.join(lines[1:])
    body_md = re.sub(r'^---\s*$', '', body_md, flags=re.MULTILINE)
    body_md = re.sub(r'\|', ' ', body_md)

    # Extract article number for cover image
    article_num = re.match(r'(\d+)', f)
    cover = f'{article_num.group(1)}_cover.png' if article_num else '01_cover.png'

    # Estimate reading time (Chinese ~400 chars/min)
    char_count = len(raw)
    readtime = max(1, round(char_count / 400))

    body_normalized = normalize_markdown(body_md)
    toc = extract_toc(body_normalized)
    toc_html = generate_toc_html(toc)
    faq_schema = generate_faq_schema(faq)
    related_html = generate_related_html(related, files, META, f, region, tag, limit=5)
    tldr_html = generate_tldr(desc, faq)
    body_html = md_to_html(body_md)

    html = HTML_ARTICLE.format(
        title=title, desc=desc, region=region, tag=tag,
        date=date, last_updated=LAST_UPDATED, slug=slug, cover=cover, readtime=readtime,
        tldr=tldr_html, toc=toc_html, body=body_html,
        faq_schema=faq_schema, related=related_html,
    )

    out = os.path.join(ARTICLES_OUT, f'{slug}.html')
    with open(out, 'w', encoding='utf-8') as fh:
        fh.write(html)
    actual_related = related_html.count('<li>')
    print(f'  ✅ {slug}.html (FAQ:{len(faq)} TL;DR✓ 内链:{actual_related})')


# ===== Articles Index =====
REGION_ORDER = ['柬埔寨', '泰国', '马来西亚', '罗马尼亚', '越南', '印尼', '东盟', '跨境']
region_groups = {r: [] for r in REGION_ORDER}
tag_groups = {}
for f in files:
    meta = META.get(f, {})
    slug = meta.get('slug', '')
    desc = meta.get('desc', '')
    region = meta.get('region', '跨境')
    tag = meta.get('tag', '法律')
    path = os.path.join(ARTICLES_DIR, f)
    with open(path, 'r', encoding='utf-8') as fh:
        title = fh.readline().replace('# ', '').strip()
    card = f'''    <a href="/articles/{slug}.html" class="article-card">
      <div class="card-meta">{region} · {tag}<span class="card-date">{meta.get('date','')}</span></div>
      <div class="card-title">{title}</div>
      <div class="card-desc">{desc}</div>
    </a>'''
    if region in region_groups:
        region_groups[region].append(card)
    else:
        region_groups['跨境'].append(card)
    # Tag groups
    if tag not in tag_groups:
        tag_groups[tag] = []
    tag_groups[tag].append(card)

cards_html = ''
region_counts = []
for r in REGION_ORDER:
    if region_groups[r]:
        count = len(region_groups[r])
        region_counts.append(f'{r}({count}篇)')
        cards_html += f'<h3 class="region-heading" id="region-{r}">{r} · {count} 篇</h3>\n'
        cards_html += '\n'.join(region_groups[r]) + '\n'

# Tag pills
tag_pills = ''
TAG_ORDER = ['投资法','税务合规','数据合规','劳动法','争议解决','刑事合规','数字经济','工程法','并购','公司合规','房地产法','知识产权','行政合规','金融监管','能源法','土地法','环境法','外汇管理','破产重整','反洗钱','公司法','ODI','税务筹划','基础设施','行业准入','贸易法','移民法','农业投资','保险监管','外资审查','地缘投资','刑事辩护','竞争法']
for t in TAG_ORDER:
    if t in tag_groups:
        count = len(tag_groups[t])
        tag_pills += f'<a href="#tag-{t}" class="tag-pill">{t}<span class="tag-count">{count}</span></a>\n'

# Tag sections
tag_sections = ''
for t in TAG_ORDER:
    if t in tag_groups:
        tag_sections += f'<h3 class="region-heading" id="tag-{t}">{t} · {len(tag_groups[t])} 篇</h3>\n'
        tag_sections += '\n'.join(tag_groups[t]) + '\n'

with open(os.path.join(ARTICLES_OUT, 'index.html'), 'w', encoding='utf-8') as fh:
    fh.write(f'''<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="baidu-site-verification" content="codeva-clmOQd5j7H" />
<title>跨境投资法律实务文章合集 | 余驰宇律师</title>
<meta name="description" content="余驰宇律师跨境投资法律实务文章合集，覆盖泰国、柬埔寨、马来西亚、罗马尼亚、越南、印尼六国，共{len(files)}篇专业深度分析。">
<meta property="og:title" content="跨境投资法律实务文章合集 | 余驰宇律师">
<meta property="og:description" content="覆盖泰柬马罗越印六国，{len(files)}篇跨境投资法律深度文章。">
<meta property="og:image" content="https://chiryyu.com/images/01_cover.png">
<meta property="og:url" content="https://chiryyu.com/articles/">
<meta property="og:type" content="website">
<meta property="og:site_name" content="余驰宇律师 | 跨境投资法律实务">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="跨境投资法律实务文章合集 | 余驰宇律师">
<meta name="twitter:description" content="覆盖泰柬马罗越印六国，{len(files)}篇跨境投资法律深度文章。">
<meta name="twitter:image" content="https://chiryyu.com/images/01_cover.png">
<link rel="stylesheet" href="/css/style.css">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-RNSF9MHKRC"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){{dataLayer.push(arguments);}}
  gtag('js', new Date());
  gtag('config', 'G-RNSF9MHKRC');
</script>
<script>
var _hmt = _hmt || [];
(function() {{
  var hm = document.createElement("script");
  hm.src = "https://hm.baidu.com/hm.js?4f699fb9163cce274dc69ba8316bb3e8";
  var s = document.getElementsByTagName("script")[0];
  s.parentNode.insertBefore(hm, s);
}})();
</script>
</head>
<body>
<header class="site-header">
  <div class="header-inner">
    <a href="/" class="site-title">余驰宇<span>律师</span></a>
    <nav class="site-nav"><a href="/">首页</a><a href="/articles/" class="active">文章</a><a href="/cambodia-lawyer.html">柬埔寨律师</a><a href="/about.html">关于</a></nav>
  </div>
</header>
<main class="main-content">
  <h2 class="section-title">全部文章（共 {len(files)} 篇）</h2>
  <p class="region-summary">{' · '.join(region_counts)}</p>
  <div class="tag-cloud">{tag_pills}</div>
  <div class="view-toggle" style="text-align:center;margin:1.5rem 0 2rem;">
    <a href="#region-柬埔寨" class="view-btn active" onclick="switchView(event,'region')">按法域浏览</a>
    <a href="#tag-投资法" class="view-btn" onclick="switchView(event,'tag')">按主题浏览</a>
  </div>
  <div id="region-view">{cards_html}</div>
  <div id="tag-view" style="display:none;">{tag_sections}</div>
</main>
<script>
function switchView(e,view) {{
  document.querySelectorAll('.view-btn').forEach(function(b){{b.classList.remove('active')}});
  e.target.classList.add('active');
  if(view==='region'){{document.getElementById('region-view').style.display='block';document.getElementById('tag-view').style.display='none'}}
  else{{document.getElementById('region-view').style.display='none';document.getElementById('tag-view').style.display='block'}}
}}
</script>
<footer class="site-footer">
  <p>© 2026 余驰宇律师 · <a href="/">chiryyu.com</a></p>
  <p style="margin-top:0.5rem;">📞 <a href="tel:+8615201911206">15201911206</a> · 微信 chiry003</p>
</footer>
</body>
</html>''')
print('  ✅ articles/index.html')

# ===== About page =====
with open(os.path.join(SITE_DIR, 'about.html'), 'w', encoding='utf-8') as fh:
    fh.write('''<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="baidu-site-verification" content="codeva-clmOQd5j7H" />
<title>关于余驰宇律师 | 上海涉外律师 · 跨境投资法律实务</title>
<meta name="description" content="余驰宇，中国执业律师，大成律师事务所，专注东南亚与东欧跨境投资法律实务。覆盖泰国、柬埔寨、马来西亚、罗马尼亚、越南、印尼六国。电话：15201911206。">
<meta property="og:title" content="余驰宇律师 | 上海涉外律师 · 跨境投资法律实务">
<meta property="og:description" content="中国执业律师，大成律师事务所，专注东南亚与东欧跨境投资法律实务，覆盖泰柬马罗越印六国。">
<meta property="og:image" content="https://chiryyu.com/images/avatar.png">
<meta property="og:url" content="https://chiryyu.com/about.html">
<meta property="og:type" content="profile">
<meta property="og:site_name" content="余驰宇律师 | 跨境投资法律实务">
<meta name="twitter:card" content="summary">
<meta name="twitter:title" content="余驰宇律师 | 上海涉外律师">
<meta name="twitter:description" content="中国执业律师，专注东南亚与东欧跨境投资法律实务。">
<meta name="twitter:image" content="https://chiryyu.com/images/avatar.png">
<link rel="stylesheet" href="/css/style.css">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-RNSF9MHKRC"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', 'G-RNSF9MHKRC');
</script>
<script>
var _hmt = _hmt || [];
(function() {
  var hm = document.createElement("script");
  hm.src = "https://hm.baidu.com/hm.js?4f699fb9163cce274dc69ba8316bb3e8";
  var s = document.getElementsByTagName("script")[0];
  s.parentNode.insertBefore(hm, s);
})();
</script>
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Person",
  "name": "余驰宇",
  "jobTitle": "律师",
  "hasCredential": "中国执业律师",
  "description": "中国执业律师，大成律师事务所，牵头柬埔寨、泰国、马来西亚、罗马尼亚等国当地律所的中国业务部。专注中资企业海外财产保护、重大诉讼/仲裁、税务问题处理、跨境保全与执行。",
  "url": "https://chiryyu.com",
  "image": "https://chiryyu.com/images/avatar.png",
  "telephone": "+8615201911206",
  "affiliation": { "@type": "Organization", "name": "大成律师事务所", "url": "https://shanghai.dacheng.com/lawyer_1/43.html", "address": {"@type":"PostalAddress","addressLocality":"上海","addressCountry":"CN"} },
  "memberOf": { "@type": "Organization", "name": "大成律师事务所", "url": "https://shanghai.dacheng.com/lawyer_1/43.html" },
  "knowsAbout": ["柬埔寨投资法","柬埔寨QIP","柬埔寨税务","柬埔寨土地法","柬埔寨劳动法","柬埔寨律师","泰国法律","泰国BOI投资","泰国PDPA","马来西亚法律","罗马尼亚法律","越南法律","印尼法律","跨境投资法","国际仲裁","税务合规","数据保护","东盟法律"],
  "areaServed": [{"@type":"Country","name":"柬埔寨"},{"@type":"Country","name":"泰国"},{"@type":"Country","name":"马来西亚"},{"@type":"Country","name":"罗马尼亚"},{"@type":"Country","name":"越南"},{"@type":"Country","name":"印尼"},{"@type":"Country","name":"中国"}],
  "knowsLanguage": ["zh-CN", "en"],
  "address": {"@type":"PostalAddress","addressLocality":"上海","addressCountry":"CN"},
  "sameAs": ["https://shanghai.dacheng.com/lawyer_1/43.html", "https://chiryyu.com/about.html"]
}
</script>
</head>
<body>
<header class="site-header">
  <div class="header-inner">
    <a href="/" class="site-title">余驰宇<span>律师</span></a>
    <nav class="site-nav"><a href="/">首页</a><a href="/articles/">文章</a><a href="/cambodia-lawyer.html">柬埔寨律师</a><a href="/about.html" class="active">关于</a></nav>
  </div>
</header>
<section class="about-hero">
  <img src="/images/avatar.png" alt="余驰宇律师" class="about-avatar" width="120" height="120">
  <h1>余驰宇</h1><p>中国执业律师 · 跨境投资法律实务</p>
</section>
<main class="about-content">
  <h2>执业领域</h2>
  <p>余驰宇，中国执业律师，大成律师事务所，牵头柬埔寨、泰国、马来西亚、罗马尼亚等国当地律所的中国业务部，专注中资企业海外财产保护、重大诉讼/仲裁、税务问题处理、跨境保全与执行。</p>
  <h2>覆盖法域</h2>
  <ul>
    <li><strong>柬埔寨</strong> — QIP投资法、税务制度、土地法、劳动法、知识产权、外汇管理、环境法、经济特区、数字经济、破产法、建筑施工、银行金融监管、赌场牌照</li>
    <li><strong>泰国</strong> — BOI投资、税务争议、M&A并购、PDPA数据合规、破产重整、工厂法、工作签证社保、外国人购房、刑事辩护、EPC工程、反洗钱法</li>
    <li><strong>马来西亚</strong> — AMLA反洗钱、竞争法并购管制、PDPA数据合规、劳动法外籍员工、土地法、特许经营、中国仲裁执行</li>
    <li><strong>罗马尼亚</strong> — GDPR数据保护、劳动法、公司法外商投资、民事诉讼、储能投资、外资审查、国家援助规则</li>
    <li><strong>越南</strong> — 投资法外资准入、劳动法、税务转让定价、数据本地化、跨境传输合规</li>
    <li><strong>印尼</strong> — Omnibus综合法、IKN新首都投资、PDP数据保护法、外资准入</li>
    <li><strong>跨境</strong> — ODI备案全流程、双边税收协定税务筹划、FIDIC国际工程合同、东盟跨境电商合规、东盟六国仲裁、一带一路风险地图</li>
  </ul>
  <h2>权威数据来源</h2>
  <p style="color:var(--gray-400);font-size:0.95rem;">本文站所有文章基于以下一手法律来源与官方数据：</p>
  <ul style="color:var(--gray-400);font-size:0.9rem;">
    <li>柬埔寨发展理事会（CDC）投资审批年报与QIP公告</li>
    <li>柬埔寨税务总局（GDT）税务稽查年报与Prakas公告</li>
    <li>泰国投资促进委员会（BOI）公告与投资促进法修订案</li>
    <li>泰国个人数据保护委员会（PDPC）执法年报</li>
    <li>马来西亚投资发展局（MIDA）投资统计与政策公告</li>
    <li>马来西亚竞争委员会（MyCC）并购申报指引与执法年报</li>
    <li>罗马尼亚财政部PNRR执行进度报告</li>
    <li>欧盟委员会罗马尼亚国家报告（European Semester）</li>
    <li>世界银行东南亚经济展望报告（2025）</li>
    <li>各国双边投资协定（BIT）与避免双重征税协定（DTA）官方文本</li>
    <li>各国最高法院与宪法法院判例数据库</li>
    <li>OECD转让定价指南（2022年版）</li>
  </ul>
  <h2>联系方式</h2>
  <p>📞 电话：<a href="tel:+8615201911206">15201911206</a></p>
  <p>💬 微信：<strong>chiry003</strong></p>
  <p>网站：<a href="https://shanghai.dacheng.com/lawyer_1/43.html">大成律师事务所</a></p>
  <p style="margin-top:1rem;font-size:0.9rem;color:var(--gray-500);"><a href="/cambodia-lawyer.html">柬埔寨律师专题页</a> · <a href="https://www.cambodiainvestment.gov.kh/" target="_blank" rel="noopener">CDC官网</a> · <a href="https://www.boi.go.th/" target="_blank" rel="noopener">BOI官网</a> · <a href="https://www.mida.gov.my/" target="_blank" rel="noopener">MIDA官网</a></p>
</main>
<footer class="site-footer">
  <p>© 2026 余驰宇律师 · <a href="/">chiryyu.com</a></p>
  <p style="margin-top:0.5rem;">📞 <a href="tel:+8615201911206">15201911206</a> · 微信 chiry003</p>
</footer>
</body>
</html>''')
print('  ✅ about.html')

# ===== Homepage =====
# Latest 5 articles
latest_articles = []
for f in files:
    meta = META.get(f, {})
    if not meta: continue
    date = meta.get('date', '')
    latest_articles.append((date, meta.get('slug',''), meta.get('region',''), meta.get('tag',''), meta.get('desc',''), f))
latest_articles.sort(key=lambda x: x[0], reverse=True)
latest_5 = latest_articles[:5]
latest_html = ''
for date, slug, region, tag, desc, fname in latest_5:
    path = os.path.join(ARTICLES_DIR, fname)
    title = ''
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as fh:
            title = fh.readline().replace('# ', '').strip()
    latest_html += f'''    <a href="/articles/{slug}.html" class="article-card">
      <div class="card-meta">{region} · {tag}<span class="card-date">{date}</span></div>
      <div class="card-title">{title}</div>
      <div class="card-desc">{desc}</div>
    </a>\n'''

# Region nav cards
LAWYER_PAGES = [
    ('柬埔寨', '24篇', '/cambodia-lawyer.html', 'QIP申请、土地法、税务、劳动法、经济特区'),
    ('泰国', '11篇', '/thailand-lawyer.html', 'BOI申请、税务争议、代持化解、PDPA、M&A'),
    ('马来西亚', '13篇', '/malaysia-lawyer.html', 'JS-SEZ、数据中心、并购管制、PDPA、AMLA'),
    ('罗马尼亚', '10篇', '/romania-lawyer.html', '欧盟资金、GDPR、可再生能源、乌克兰重建'),
    ('越南', '7篇', '/vietnam-lawyer.html', '外资准入、数据本地化、税务转让定价、VIAC'),
    ('印尼', '3篇', '/indonesia-lawyer.html', 'Omnibus综合法、IKN新首都、PDP数据保护'),
    ('跨境投资', '6篇', '/crossborder-lawyer.html', 'ODI备案、税务筹划、FIDIC工程、国际仲裁'),
]
region_nav = ''
for name, count, url, topics in LAWYER_PAGES:
    region_nav += f'''      <a href="{url}" class="region-card">
        <strong>{name}<span class="region-card-count">{count}</span></strong>
        <p>{topics}</p>
      </a>\n'''

total = len(files)
regions_count = len([r for r in REGION_ORDER if region_groups.get(r)])

with open(os.path.join(SITE_DIR, 'index.html'), 'w', encoding='utf-8') as fh:
    fh.write(f'''<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="icon" type="image/x-icon" href="/favicon.ico">
<link rel="icon" type="image/png" sizes="32x32" href="/images/favicon-32x32.png">
<link rel="apple-touch-icon" sizes="180x180" href="/images/apple-touch-icon.png">
<meta name="baidu-site-verification" content="codeva-clmOQd5j7H" />
<title>余驰宇律师 | 跨境投资法律实务</title>
<meta name="description" content="余驰宇，中国执业律师，大成律师事务所。覆盖柬埔寨、泰国、马来西亚、罗马尼亚、越南、印尼六国。{total}篇跨境投资法律深度文章。电话15201911206。">
<meta name="keywords" content="跨境投资律师,柬埔寨律师,泰国律师,马来西亚律师,罗马尼亚律师,越南律师,印尼律师,境外投资法律,海外合规,上海涉外律师,余驰宇">
<meta name="author" content="余驰宇">
<link rel="canonical" href="https://chiryyu.com/">
<meta property="og:title" content="余驰宇律师 | 跨境投资法律实务">
<meta property="og:description" content="中国执业律师，覆盖六国+跨境，{total}篇深度文章。专注海外财产保护、国际仲裁、税务合规、跨境保全与执行。">
<meta property="og:image" content="https://chiryyu.com/images/avatar.png">
<meta property="og:url" content="https://chiryyu.com/">
<meta property="og:type" content="website">
<meta property="og:site_name" content="余驰宇律师 | 跨境投资法律实务">
<meta property="og:locale" content="zh_CN">
<meta name="twitter:card" content="summary_large_image">
<link rel="stylesheet" href="/css/style.css">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-RNSF9MHKRC"></script>
<script>window.dataLayer=window.dataLayer||[];function gtag(){{dataLayer.push(arguments);}}gtag('js',new Date());gtag('config','G-RNSF9MHKRC');</script>
<script>var _hmt=_hmt||[];(function(){{var hm=document.createElement("script");hm.src="https://hm.baidu.com/hm.js?4f699fb9163cce274dc69ba8316bb3e8";var s=document.getElementsByTagName("script")[0];s.parentNode.insertBefore(hm,s);}})();</script>
<script type="application/ld+json">
{{
  "@context":"https://schema.org",
  "@type":"Person",
  "name":"余驰宇","alternateName":["Yu Chiyu","余驰宇律师"],
  "jobTitle":"中国执业律师","hasCredential":"中国执业律师",
  "description":"中国执业律师，大成律师事务所，牵头柬埔寨、泰国、马来西亚、罗马尼亚等国当地律所的中国业务部。专注中资企业海外财产保护、重大诉讼/仲裁、税务问题处理、跨境保全与执行。",
  "url":"https://chiryyu.com","image":"https://chiryyu.com/images/avatar.png","telephone":"+8615201911206",
  "affiliation":{{"@type":"Organization","name":"大成律师事务所","url":"https://shanghai.dacheng.com/lawyer_1/43.html"}},
  "memberOf":{{"@type":"Organization","name":"大成律师事务所","url":"https://shanghai.dacheng.com/lawyer_1/43.html"}},
  "knowsAbout":["柬埔寨律师","泰国律师","马来西亚律师","罗马尼亚律师","跨境投资法","国际仲裁","税务合规","数据保护"],
  "areaServed":[{{"@type":"Country","name":"柬埔寨"}},{{"@type":"Country","name":"泰国"}},{{"@type":"Country","name":"马来西亚"}},{{"@type":"Country","name":"罗马尼亚"}},{{"@type":"Country","name":"越南"}},{{"@type":"Country","name":"印尼"}},{{"@type":"Country","name":"中国"}}],
  "knowsLanguage":["zh-CN","en"],
  "address":{{"@type":"PostalAddress","addressLocality":"上海","addressCountry":"CN"}},
  "sameAs":["https://shanghai.dacheng.com/lawyer_1/43.html","https://chiryyu.com/about.html"]
}}
</script>
<script type="application/ld+json">
{{
  "@context":"https://schema.org",
  "@type":"Organization",
  "name":"大成律师事务所","alternateName":"Dacheng Law Offices",
  "url":"https://shanghai.dacheng.com/lawyer_1/43.html",
  "member":{{"@type":"Person","name":"余驰宇","jobTitle":"中国执业律师","url":"https://chiryyu.com"}},
  "address":{{"@type":"PostalAddress","addressLocality":"上海","addressCountry":"CN"}},
  "contactPoint":{{"@type":"ContactPoint","contactType":"Legal Services","telephone":"+8615201911206","areaServed":["柬埔寨","泰国","马来西亚","罗马尼亚","越南","印尼","中国"]}}
}}
</script>
<script type="application/ld+json">
{{
  "@context":"https://schema.org",
  "@type":"LegalService",
  "name":"余驰宇律师 | 跨境投资法律实务",
  "image":"https://chiryyu.com/images/avatar.png",
  "url":"https://chiryyu.com","telephone":"+8615201911206",
  "description":"中国执业律师，专注东南亚与东欧跨境投资法律实务。",
  "address":{{"@type":"PostalAddress","addressLocality":"上海","addressCountry":"CN"}},
  "areaServed":["柬埔寨","泰国","马来西亚","罗马尼亚","越南","印尼","中国"],
  "hasCredential":"中国执业律师"
}}
</script>
<script type="application/ld+json">
{{
  "@context":"https://schema.org","@type":"WebSite",
  "name":"余驰宇律师 | 跨境投资法律实务",
  "url":"https://chiryyu.com",
  "description":"中国执业律师，专注东南亚与东欧跨境投资法律实务",
  "potentialAction":{{"@type":"SearchAction","target":"https://chiryyu.com/articles/?q={{search_term_string}}","query-input":"required name=search_term_string"}}
}}
</script>
</head>
<body>
<header class="site-header">
  <div class="header-inner">
    <a href="/" class="site-title">余驰宇<span>律师</span></a>
    <nav class="site-nav"><a href="/" class="active">首页</a><a href="/articles/">文章</a><a href="/cambodia-lawyer.html">柬埔寨律师</a><a href="/about.html">关于</a></nav>
  </div>
</header>

<section class="hero">
  <h1>跨境投资法律<span>实务</span></h1>
  <p class="subtitle">中国执业律师，大成律师事务所。深耕柬埔寨、泰国、马来西亚、罗马尼亚等国跨境投资法律实务，牵头当地律所中国业务部，专注中资企业海外财产保护、重大诉讼/仲裁、税务问题处理、跨境保全与执行。</p>
  <div class="regions">
    <span class="region-tag">柬埔寨</span><span class="region-tag">泰国</span><span class="region-tag">马来西亚</span><span class="region-tag">罗马尼亚</span><span class="region-tag">越南</span><span class="region-tag">印尼</span>
  </div>
  <div class="stats">
    <span>{total} 篇专业文章</span><span>覆盖 {regions_count} 个司法辖区</span><span>100万+ 字深度分析</span>
  </div>
</section>

<section class="service-intro">
  <h2>服务领域</h2>
  <div class="service-grid">
    <div class="service-item"><strong>跨境投资架构</strong><p>ODI备案、外资准入审查、控股架构设计、双边税收协定筹划</p></div>
    <div class="service-item"><strong>海外公司治理</strong><p>公司设立、劳动法合规、股权代持风险排查、BOI/QIP申请</p></div>
    <div class="service-item"><strong>税务与转让定价</strong><p>跨境税务筹划、转让定价文档、税务稽查应对、全球最低税评估</p></div>
    <div class="service-item"><strong>争议解决</strong><p>国际仲裁（SIAC/HKIAC/ICC）、跨境诉讼保全、裁决承认与执行</p></div>
    <div class="service-item"><strong>数据与合规</strong><p>PDPA/GDPR/PDP合规、数据跨境传输、反洗钱AML体系建设</p></div>
    <div class="service-item"><strong>房地产与工程</strong><p>外国人购房合规、FIDIC合同谈判、工程款纠纷、施工许可</p></div>
  </div>
</section>

<section class="region-navigator">
  <h2 class="section-title">按法域浏览</h2>
  <div class="region-card-grid">
{region_nav}  </div>
</section>

<main class="main-content">
  <h2 class="section-title">最新文章</h2>
{latest_html}  <p style="text-align:center;margin-top:2rem;">
    <a href="/articles/" style="color:var(--gold-dark);text-decoration:none;font-weight:600;">查看全部 {total} 篇文章 →</a>
  </p>
</main>

<footer class="site-footer">
  <p>© 2026 余驰宇律师 · <a href="/">chiryyu.com</a></p>
  <p style="margin-top:0.5rem;">📞 <a href="tel:+8615201911206">15201911206</a> · 微信 chiry003</p>
</footer>
</body>
</html>''')
print('  ✅ index.html (首页)')

# ===== robots.txt =====
with open(os.path.join(SITE_DIR, 'robots.txt'), 'w') as fh:
    fh.write('''User-agent: *
Allow: /
Sitemap: https://chiryyu.com/sitemap.xml
User-agent: GPTBot
Allow: /
User-agent: CCBot
Allow: /
User-agent: anthropic-ai
Allow: /
User-agent: Google-Extended
Allow: /
User-agent: Bytespider
Allow: /
User-agent: Baiduspider
Allow: /
''')
print('  ✅ robots.txt')

# ===== sitemap =====
urls = [
    ('https://chiryyu.com/', 'weekly', '1.0', '2026-06-22'),
    ('https://chiryyu.com/about.html', 'monthly', '0.8', '2026-06-22'),
    ('https://chiryyu.com/cambodia-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/thailand-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/malaysia-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/vietnam-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/indonesia-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/romania-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/crossborder-lawyer.html', 'weekly', '0.9', '2026-08-17'),
    ('https://chiryyu.com/articles/', 'weekly', '0.9', '2026-06-22'),
]
for f in files:
    meta = META.get(f, {})
    date = meta.get('date', '2026-06-01')
    urls.append((f"https://chiryyu.com/articles/{meta.get('slug', '')}.html", 'monthly', '0.7', date))
with open(os.path.join(SITE_DIR, 'sitemap.xml'), 'w') as fh:
    fh.write('<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n')
    for url, freq, pri, lastmod in urls:
        fh.write(f'  <url><loc>{url}</loc><lastmod>{lastmod}</lastmod><changefreq>{freq}</changefreq><priority>{pri}</priority></url>\n')
    fh.write('</urlset>\n')
print('  ✅ sitemap.xml')

with open(os.path.join(SITE_DIR, 'CNAME'), 'w') as fh:
    fh.write('chiryyu.com')

print(f'\n✅ 全部20篇文章 GEO 优化完成！')
print(f'   - FAQ 结构化数据（AI搜索引擎优先抓取）')
print(f'   - TL;DR 摘要（每篇开头）')
print(f'   - 交叉内链（每篇3-4篇相关文章）')
print(f'   - 百度/Google 双统计')
print(f'   - robots.txt 开放所有 AI 爬虫')
